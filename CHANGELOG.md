...
- **Video requests now carry a frame-count knob across the compat vocabulary.** `/v1/videos/generations` accepted only sd-server's native `video_frames` key — the OpenAI-style `frames`/`num_frames` synonyms and the REPL's `/duration` seconds knob passed through unvalidated and unmapd, so those callers silently got whatever frame count the child defaulted to. The video handler now canonicalizes the whole vocabulary onto `video_frames` before relay: `frames`/`num_frames` map directly, `duration` translates through the request `fps` (default 16, the child's own), values below 1 are refused with a teaching 400, and conflicting spellings (`frames: 4` + `video_frames: 8`, or a `duration` that disagrees with an explicit count) are named in the error instead of racing. Wan-family alignment (4k+1 temporal grid) still belongs to the engine — a request for 4 frames arrives as `video_frames: 4` and the child aligns it; the gateway teaches the vocabulary, not the model's math.
