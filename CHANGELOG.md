...
All notable changes to Blazar are documented here. Format follows
...
- **The video VRAM gate prices an unspecified frame count at the calibrated horizon, not 1.** A bare video request (no frames/duration) used to be priced for a single frame while the child's own multi-frame default rendered far more — an under-priced estimate can OOM mid-render on small cards. Absent frame specs now price at the 33-frame calibration horizon; explicit counts (including 1) still price exactly as asked.

- **Media job polling survives transient hiccups and names its budget.** A single dropped poll request used to kill a healthy minute-25 generation (immediate 502 + reap); three consecutive transport failures (~3 s) are now required before the child is declared dead, and a 404 stays terminal as before. The gateway-side wait for sync/stream media jobs is capped by a new `media_job_wait_secs` knob (default 900, `0` = uncapped, else 5..=86400) — on expiry the client gets an error naming the knob and the async-handle alternative instead of a silent close, and async job handles were never capped and still are not. Expiry now also cancels the still-rendering child job (best-effort) on both the await and stream paths — a timed-out client no longer leaves the GPU rendering for nobody; live-verified against the child's job table (cancelled, then reset).

- **Abandoning a streaming generation cancels the child job.** A client disconnect on an `stream: true` image/video generation used to leave the engine rendering to completion for nobody; the gateway now fires the child's cancel endpoint (best-effort) on the disconnect path. Stream-end error paths (budget exceeded, job vanished, child death) now emit proper `event: error` SSE frames instead of just closing.

- **REPL media knobs: `/seed`, `/negative`, `/frames`.** Image and video REPLs only exposed `/size` (`/steps` on image, `/duration` on video); seed, negative prompt, and frame count existed only in the API's rich dialect. All three are now REPL knobs (`/seed -1` re-rolls per request, bare `/negative` clears), and requests that omit them stay byte-identical to the plain compat dialect.

- **sdcpp children generate nondeterministically and at family-correct quality.** Every sd-server spawn inherited the engine's SD1.5-era defaults: seed 42 (two identical requests returned byte-identical images — live receipt: 0/262144 pixels differ), cfg 7.0 with the `euler_a` sampler at 512x512, which washes out flow-matching DiTs like qwen-image (native 1024, `true_cfg` 4.0). The launch line now carries `--seed -1` on every spawn (upstream: negative seed re-rolls per request) and, when the component set marks the qwen-image family (`--llm` encoder), its documented posture: `--cfg-scale 4.0 --sampling-method euler --flow-shift 3.0 --width 1024 --height 1024`. Wan video and flux keep their engine model-type defaults (seed only). Every flag rides the probed-manifest gate (an older engine degrades with a warning, never a dead child), user `extra_args` land after the defaults so sd-server's last-one-wins parsing takes the user's value, and explicit request parameters still override both dialects. Also fixed en route: threads tuning emitted the short `-t` spelling the long-flag-only manifest gate always refused, so `threads` never reached the launch line (live child argv carried no flag at all); it now emits `--threads`, and `--threads`/`-t` are reserved launch pins in `extra_args`.

- **Diffusion lanes stop being evicted mid-generation by the text-lane header ceiling.** A diffusion child sends its first response byte only when the whole frame is computed, so time-to-headers IS generation time — minutes at 1024px on offloaded hardware, longer for video. The shared `child_header_timeout_secs` ceiling (120 s, sized for text prefill) therefore killed healthy generations live: a qwen-image tour was evicted 120 s in and the client ate a 502 after the in-band retry re-ran the same race. The header bound is now lane-aware — `sdcpp_child_header_timeout_secs` (default 900, `0` off or `5..=3600`, same evict-and-retry-once semantics) applies to sdcpp children while text lanes keep their knob — and the eviction warning names which ceiling fired.

- **Routed-adapter spawns anchor relative engine rows again.** The routed-adapter decode site (a model whose stored engine kind differs from the serving lane, e.g. a diffusion model registered under llamacpp) still re-rooted the manifest's server path the legacy way — correct for absolute rows, a no-op for the data-dir-relative rows the boot pass now writes — so a freshly migrated row exec'd `engines/<tag>/sd-server` raw and every routed spawn died ENOENT with a 500. The site now anchors through the same primitive the boot path uses (relative rows join the live data dir; legacy absolute rows keep the re-root heal), and routed spawns behave exactly like daemon-booted ones again.

### Added

- **GitHub engine downloads ride the parallel byte-range lane.** Release assets — llama.cpp binaries, mistral.rs tarballs, sdcpp, CUDA runtime companions (0.1–1.1 GiB) — downloaded single-stream before: no parallelism, no resume, a killed install restarted from byte zero. Assets with a known size now probe the host with a strict `Range: bytes=0-0` request and, when the CDN answers a clean 206, download through the same chunked machinery model pulls use: 8 connections on 8 MiB chunks, a per-chunk resume ledger (an interrupted install continues where it stopped), full sha256 verification against the release digest, and atomic finalize. The release-API size is enforced after the transfer — a CDN that under-delivers or metadata that lies fails loudly with no partial file surviving. Range-incapable hosts and small assets fall back to the classic single stream, byte-identical to before. Pinned by wire-shape tests: probe-plus-chunks on the wire, 200-only fallback, and the lying-metadata guard.

- **First-class sdcpp tuning knobs: cache mode, flash attention, VAE tiling, RPC offload.** sd-server ships major levers — DiT step-caching accelerators (`--cache-mode` easycache/ucache/dbcache/taylorseer/cache-dit/spectrum), flash attention (`--fa`), tiled VAE decode for small VRAM, and RPC weight offload (`--rpc-servers`, matching the llama.cpp lane's `--rpc` parity) — but they were reachable only through per-model `extra_args`, undiscoverable. Four config fields now expose them globally: `sdcpp_cache_mode` (validated against the six upstream modes), `sdcpp_flash_attention`, `sdcpp_vae_tiling`, and `sdcpp_rpc_servers` (host:port list). Emission rides the probed-manifest gate (an engine lacking a flag degrades with a warning, never a dead child), and user `extra_args` land after them so sd-server's last-one-wins parsing keeps the user's value final. Flash attention ships **on by default** — a seeded A/B on qwen-image (1024², 20 steps) measured 25% faster renders (301s → 224s) at statistically identical output (five-metric stats within ~1%, mean pixel delta 0.46% of range) — while cache mode, VAE tiling, and RPC stay opt-in (cache mode measurably trades detail for speed: 2.2x with −34% gradient energy). Live receipt: the flags verifiably present in the spawned child's argv, and `easycache` + `--fa` cut a seeded render from 319 s to 141 s (>2.2x) including cold weight load.

- **Updates restart a running daemon so the change is live immediately.** A CLI upgrade, engine update/build/use/local registration, or `blazar tune` adoption used to end with "restart the daemon to apply" — the daemon kept serving the old binary, old engine, old tuning until a manual stop. Every such command now restarts the daemon itself when one is provably running, through the narrowest lane available: `systemctl restart` when the unit is active, `launchctl kickstart -k` on macOS, and a privilege-free stop-and-respawn (SIGTERM, wait out the graceful drain — including minutes-long in-flight image generations holding the daemon lock — then relaunch through the same auto-start path) everywhere else. A restart that cannot land is a warning naming the manual remedy, never a failed update; no command grows a daemon-start side effect; re-selecting the same active engine is a no-op. Pinned end to end: the systemd lane is exercised by a hermetic shim (PATH-fronted `systemctl` whose verdicts pick the lane) against a stub healthz server, and the drain-wait by owner-alive/exit unit pins; the stop lane's spawn races the drain by design and now waits for process exit instead of the listener alone.

- **Adaptive reshape drains gracefully instead of starving under sustained load.** The reshape used to wait for a global idle that continuous traffic never produces (live receipt: 20260924-all-engines, flat 39 t/s at C=8 for a whole campaign — the drain skipped every tick). Now a busy drain tick marks the model `reshaping` (the admission gate parks new requests on that mark and feeds the pressure gauge), and the first idle tick evicts, respawns at the adopted shape, clears the mark, and publishes `SlotsReshaped` — the gateway's wake task releases every parked waiter on that event (and on engine updates), so queued demand lands on the fresh slots immediately. A failed evict keeps the queue entry for retry; a failed respawn holds the mark one more tick instead of thrashing. Pinned by the drain-holds-while-busy unit test, which runs the real respawn path (store row, GGUF metadata, profile validation, FakeEngine child) end to end.
...
- **`validate.py --smoke`: a fast pre-commit subset of the E2E harness.** The full 227-check run is the release receipt (~10-15 min with model loads); day-to-day commits only need the signal that the common breakage classes survived — binary boots, daemon spawns and serves chat, config surface, CLI surface, golden drift. `--smoke` runs exactly those six phases (~2-4 min, one 0.5B load; latest receipt: 117 checks, 0 failed) and deliberately skips the coverage-ledger phases, which redden when the rows they audit never ran. An explicit `--phase=` still wins over the preset.
...
- **`blazar doctor` names ghost engine rows.** A db row whose engine dir was deleted underneath it (manual removal, disk cleanup) previously passed every doctor check while spawn/verify silently couldn't see the binary. Doctor now lists each ghost row with its anchored path and the heal command (`blazar engine update --kind <kind>`; whisper rows note the legacy tree still serves transcription; the `local` lane teaches the `BLAZAR_ENGINE_PATH` remedy).

- **engine-check.json writes are atomic.** The currency marker is now written via tmp+rename; a plain write racing a reader (or a second daemon generation during restart) once left two concatenated JSON objects in the file.

- **Children can serve over Unix-domain sockets (`child_transport = "unix"`).** Every llama-lane child spawns on a deterministic `run/<name>.sock` in the daemon's run directory instead of an ephemeral loopback port, and every gateway→child hop (proxy hot path, metrics scrape, warmup peg, session-bank save/restore, tokenization and embedding probes, images and batch lanes) dials the socket through a bounded per-path client pool. Sockets are swept for stale files at boot and unlinked on eviction; a path that would blow the OS `sun_path` budget fails fast with a teaching error naming the component. `tcp` stays the default and is byte-identical to before; `unix` is rejected at config validation on non-unix platforms.
...
- **RPC worker handshake failures teach the fix.** A child whose `--rpc` worker answers TCP but runs a mismatched ggml build (version-mismatch abort, handshake failure, connect abort in the dying log tail) surfaces an `EngineCrashed` payload that names the engine build, the stale worker address(es), and the remedy (restart workers from the same-era build or drop them from `rpc_servers`) — classified only when the spawn's argv actually dialed `--rpc`.
...
- **Multimodal GGUFs boot on the mistral.rs lane instead of refusing at config time.** A GGUF that carries a projector (mmproj) but not its original `config.json` — the standard shape of lmstudio-community repacks — made a raw mistral.rs child abort at startup with `multimodal GGUF requires its original config.json ... pass --tok-model-id`, even though the base model exists on the Hub under a name derivable from the file's own metadata. The launcher now derives that id (`general.basename` `Org_Model` + `general.size_label` → `Org/Model-<size>`, exactly how the packagers name the source repo) and passes `--tok-model-id` on the spawn line whenever the installed engine advertises the flag — live-proven on Qwen3.5-9B: config refusal gone, server healthy, chat 200. GGUFs whose metadata can't name a Hub repo (single-segment basenames) keep the engine's teaching error instead of a guessed download. The gateway's architecture routing is unchanged — GGUFs still prefer the llama.cpp lane; this unblocks the mistral.rs lane when it is the one serving (no llama.cpp engine installed, or an explicit engine override).

### Changed

- **Zero-cost operator protections now ship on by default: `audit_log` and `pii_scrub`.** Both were opt-in since introduction even though enabling them costs nothing measurable (one append-per-generation JSONL with 16 MiB rotation; regex redaction of secrets/PII at the logging boundary) — fresh installs now get both, and an explicit `= false` in the config is still honored exactly as written. Live-verified from a minimal config: the very first generation produces exactly one audit line with trace id, status, and latency. Defaults are pinned by a test so the measured-or-free knobs stay on and the costly ones (`sdcpp_cache_mode`, `sdcpp_sage_attn`) stay off.

- **`sdcpp_sage_attn` documentation corrected: the flag is fatal off-CUDA, not a fallback.** Live benchmarking on this box's Vulkan sd-server build killed the child at load — `SageAttention is unavailable on Vulkan1; it requires patched GGML, CUDA Toolkit 12.0 or newer, and SM80 or newer kernels` — so the docs no longer claim silent fallback to flash/default kernels: the option hard-requires a patched CUDA SM80+ engine build and stays default-off, and its config doc says exactly when it is safe to enable.

### Removed

- **The REPL no longer renders or plays media inline.** Image loops stopped drawing previews in the terminal (`/preview` and the viuer-based inline render are gone — every turn still writes the artifact and prints its path), and speech loops stopped auto-playing clips (`/play`, the detached WAV-player probe, and `blazar tts --no-play` are gone — every turn still writes the WAV and prints its path). Generation routing by model kind is untouched: image, video, speech, and text loops all behave as before, and the server-side speech peak limiter is unchanged. The `viuer` and `image` dependencies left the CLI with the feature.

## [0.11.0] - 2026-09-23

### Added

- **`--color {auto,always,never}` flags the whole CLI.** A global option that governs every ANSI accent: `auto` (default) keeps the terminal probe — color only on an interactive TTY that honors `NO_COLOR` and `TERM=dumb`; `always` forces accents on and deliberately beats `NO_COLOR` (an explicit flag is a stronger ask than an ambient variable); `never` emits plain bytes for scripted consumption and diffable logs.

- **The benchmark harness covers every engine lane, not just text.** `scripts/bench_matrix.py` grew four media families through the same sandboxed gateway as the text lanes: image (`sdcpp`, cold spawn + steps axis, PNG dims read from the IHDR header), video (`frames` axis with EBML container ground truth — lacing-aware SimpleBlock count, never an API field — plus the VRAM gate probe: an over-budget request that must be rejected in milliseconds with the teaching error), TTS (buffered WAV vs streamed PCM: TTFB to first body byte, RTF from the RIFF data chunk, and the TTFB speedup), and whisper (transcribes a WAV the piper lane just synthesized — the input provably comes from the voice under test, no fixture files). Media cells do not fake a quiet GPU: each row stamps `gpu_busy_mib` / `ram_avail_mib` / loadavg instead of asserting idle, cold is labeled `cold_request_s` (spawn + first request), and every lane keeps the argv stamping + cell-key dedup the text lanes already had. Rows land in the `### Media lanes` table with dynamic findings (PCM speedup, gate latency, frame-count mismatches flagged loudly). `--media-only` runs just those lanes (clears the provider sweep, sets every text-lane skip flag — the shorthand for a media-focused session), the image lane stamps PIL-gated perceptual metrics (RMS contrast, luma entropy, sampled unique colors; honestly skip-stamped when PIL is absent, with one audit PNG saved beside the cells), and a TTS concurrency probe fires N parallel streamed-PCM requests at one gateway to record wall-vs-sum parallel efficiency — the scalability receipt for the audio lane (measured: 3.9-of-4 on this box).

### Changed

- **CLI tables render through one adaptive-width engine, and doctor tells the truth.** `blazar show` no longer leaks Rust `Some(24)`/`None` for optional GGUF metadata (a value or `-`, key column sized to the longest key), `engine list` gains a real header with aligned columns instead of fixed-width guesses, and `search` drops the hardcoded ARCH width that let `xlm-roberta` bleed into CTX — sizes right-align with the numerics, missing values share one `-` placeholder, and no row carries trailing blanks (byte-clean for scripts). `doctor` status words go lowercase with terminal-only color (`NO_COLOR`/`TERM=dumb` safe, piped output byte-stable), the MODELS mega-line splits into `models` / `unmanaged files` / `model types` so each concern teaches its own remedy, and the footer counts honestly (`32 ok, 4 warning(s) — no failures`) instead of pairing `all checks pass` with warnings. Consistency sweep: `warning:` prefix everywhere, `Ctrl+D`, labeled `ms=` on request lines, unquoted model in drafts, humanized `MiB` progress on voice/whisper pulls.

- **Serve logs carry one timestamp under systemd.** When `JOURNAL_STREAM` marks journald-attached stdio, tracing drops its RFC3339 UTC stamp — journald already stamps every line in local time, so each entry carried two timestamps in two timezones. Foreground terminals keep the tracing stamp. The daemon-lock error also names the holder pid and prints the pidfile remedy exactly once instead of twice (context + CLI both appended it).

- **Unknown routes answer in the standard error envelope.** A request no router arm matches now returns the gateway's `{type: blazar_error, code, message}` JSON envelope — naming the method and the exact unmatched path — instead of axum's empty 404 body, and the message points at `GET /.well-known/blazar`, the always-fresh census of every mounted route. The fallback sits inside the layer stack, so auth, CORS, and access logging observe 404s exactly like any other response.

### Fixed

- **Wedged children can no longer poison in-band retries.** When a child misses the header window (`child_header_timeout_secs`), `child_send` now evicts it synchronously before surfacing the error, and every text lane (ollama chat non-stream + stream, `/v1/responses` buffered, the generic proxy) retries exactly once on the respawned child instead of hitting the same dead port. Previously the ollama lanes returned a bare 504 after one window with no recovery, and the generic proxy's eviction signal was asynchronous — the retry could land on the very child it had just asked to die.

- **Prompt preflight now bounds against the live compiled slot context.** All four text lanes (ollama chat + generate, OpenAI, Anthropic) resolve the admission context through `preflight::admission_ctx`: when a model is resident, the per-slot context the profile actually compiled (which auto-fit may have traded down — 16K total becomes 4x4096) is the ceiling a prompt must fit; only cold requests fall back to the configured context. Previously every lane compared against the configured total, so a prompt could pass preflight and then not fit the slot it would actually run in.

- **The semantic cache keys on the full generation equivalence class.** Cache identity now includes a fingerprint of the serving shape — sampling options, tools schema, tool choice, thinking directive, system prompt, and the resolved model — folded before the embedding similarity check. Two requests that differ only in message text still hit when semantically similar; two requests with different temperature, tools, or think mode never see each other's responses no matter how similar the prompts. The message payload itself stays out of the fingerprint (that is what the embedding compares).

- **Identical concurrent requests no longer occupy admission slots while waiting.** Single-flight coalescing moved ahead of slot admission on the OpenAI chat routes: the leader takes the in-flight slot and executes; duplicates wait on the single-flight mutex holding no capacity, then proceed warm on release. Previously N duplicates each held an in-flight slot for the whole leader window, starving unrelated requests while only one actually generated.

- **GPU admission is per-device with a reservation ledger.** The supervisor now tracks placement per card (keyed on the census backend device id) and holds an RAII reservation for the spawn window — reserved bytes count against the chosen card from pick to settle, so two concurrent cold loads cannot both pass admission on the same card and collide; the reservation converts to resident weight at settle and releases on spawn failure. Admission checks the candidate cards (manual pin or auto-pick) instead of the summed pool: the two-models-collide-on-one-card case the aggregate used to wave through is now refused. Tensor-split and unknown placements keep the aggregate belt; the spawn-time free-VRAM probe remains the final guard.

- **Embedding probes for the semantic cache load captive.** The embedding model spawned for a cache lookup now refuses (503 with teaching text) when loading it would require evicting resident generation models, instead of silently evicting the hot model the lookup was supposed to serve. Captive loads never queue on admission.

- **Observed streams stopped copying every chunk for the sentinel.** `SentinelFeed` carries refcounted `Bytes` into the analyzer channel — the per-chunk `Vec` allocation on the token hot path is gone; analysis sees the same bytes through a clone of the handle.
...
- **Diffusion spawns split modules across the card instead of blanket-offloading.** When the full qwen-image set (Q4 DiT + 8B text encoder) overruns the 75% VRAM ladder but the DiT+VAE pair alone fits, the planner now emits sd-server's per-module `--backend diffusion=<card>,vae=<card>,te=cpu` — the token resolved from the engine's own registration census (`--list-devices`, `Vulkan1`-style, discrete-card policy) — instead of `--offload-to-cpu` streaming every weight from RAM. Live on the 8 GiB box: warm generation 60s → 51s with the pair GPU-resident and the text encoder streaming one-shot per prompt. The split covers every external text-encoder family: `--llm` (Qwen-image, live-verified) and `--t5xxl`/`--clip_l`/`--clip_g` (flux/SD3) — upstream maps its old `--clip-on-cpu` flag to `--backend te=cpu`, proving `te` is the universal text-encoder module (source-verified in `examples/common/common.cpp`). Fences: an explicit `extra_args --backend` owns the posture outright (nothing stacks underneath), engines without the flag degrade to the blanket posture via the manifest gate, and standalone checkpoints keep the ladder.
...
- **`blazar doctor` model readability is lane-aware.** The models check parsed every artifact with the GGUF metadata reader, so healthy pulls warned `metadata unreadable`: HF-style safetensors shard directories, diffusers layouts (`model_index.json` plus subdirectory shards), and component-carrying DiT `.gguf` exports that ship zero metadata KVs by design (the architecture arrives via the row's `--vae`/`--llm` component set, so the LLM reader can never accept them). Readability now checks the lane the row actually serves — GGUF metadata for bare LLM weights, a structural container check plus every component present for diffusion rows, shard or `model_index.json` presence for directories. Five live false-warns reduced to zero (`11 pulled, all readable`).
...
- **Over-budget video requests are now rejected at submit time with the math shown.** The gateway estimates the generation scratch (latent + VAE working set, calibrated against measured peaks on a quiet box: flat ~3.0 GiB floor in frame count, ~3.2 KiB per pixel of resolution) plus staged weights for a cold spawn, applies a 1.25x safety factor to the uncertain terms only, and compares against live free VRAM sampled at submit. A request that cannot fit gets a 400 with the full estimate breakdown and levers (`vram_overcommit: true` to accept the risk deliberately, smaller size, fewer frames) instead of an opaque child abort minutes later. The check steps aside (not blocks) when no live census is available.

- TTS streaming: `response_format: "pcm"` on `/v1/audio/speech` streams raw s16le as it is synthesized (sentence-bounded chunks, peak-limited, `x-blazar-pcm-format` header describes the actual bytes) — time-to-first-audio measured ~3-4x faster than buffered WAV on the same input; `wav` stays buffered, other formats still teach 400.
- Idle HTTP pool churn: the three gateway reqwest clients (engine relay, OTLP, batch) now tune pool idle timeout / per-host idle / TCP keepalive instead of library defaults.
- `blazar doctor` GPU section now warns when non-blazar processes hold VRAM (with pids and MiB), since the planner budgets against free VRAM.
- REPL one-shot media artifacts no longer overwrite each other when two land in the same second: the filename stamp is now millisecond epoch plus a per-process sequence counter (`<name>-<millis>-<seq>.<ext>`), so back-to-back `blazar tts` / image one-shots each keep their own file (live-proven: two one-shots inside one wall-clock second produced two distinct WAVs).

- Benchmark harness: the blazar single-stream comparison cell ran once on a leftover loop variable (empty engine sweep made it execute against the wrong engine) — it now lives inside the per-engine loop; media engine kinds (sdcpp/whisper) are excluded from model-serving lanes; sandbox staging carries the piper engine binary (the gateway 404s "piper is not installed" without it) and whisper models.
...
- **Video generation: `POST /v1/videos/generations` bridges sd-server's native `/sdcpp/v1/vid_gen` surface.** Wan 2.1 T2V joins the curated family table as the first video family (ComfyUI-repackaged DiT + same-repo `--vae` + city96 umt5-xxl `--t5xxl`, standalone-checkpoint pull with component attachment — the same merged path SDXL pioneered, no schema change). sd-server takes no mode flag: the loaded model decides whether it serves `img_gen` or `vid_gen`, so the gateway gates the video surface off the family table — an image family posted to `/v1/videos` (and a video family posted to `/v1/images`) is refused with route teaching, both directions pinned. Job routes mount at both trees (`GET /v1/videos/jobs/{id}`, `POST /v1/videos/jobs/{id}/cancel`, `GET /v1/videos/capabilities`) — upstream runs one shared job queue, `kind: vid_gen` distinguishes the answers.
- **Image generation is a first-class served surface, not a relay.** `POST /v1/images/generations` speaks three dialects: plain OpenAI requests ride sd-server's compat endpoint byte-identical; `"async": true` submits to sd-server's native job queue and answers with poll/cancel URLs (`GET /v1/images/jobs/{id}`, `POST /v1/images/jobs/{id}/cancel`); `"stream": true` streams SSE progress frames to completion. Rich requests (cache engines, per-request LoRA strengths, guidance subtrees, VAE tiling, negative prompts) are shallow-translated to the native `/sdcpp/v1/img_gen` dialect — typed top-level renames only, every other key passes verbatim — so OpenAI callers keep their sync answer shape while reaching the full native surface. Job routes resolve live children only (never trigger a spawn; jobs die with their child and the 404 says so), and `GET /v1/images/capabilities` relays the live sampler/cache/limits catalog. The curated family table grew to nine: Qwen-Image-2.1, Qwen-Image (v1), FLUX.1, Z-Image, Chroma, FLUX.2-dev, single-checkpoint SDXL and SD 1.5 (self-contained safetensors that boot on `-m/--model` alone) and Wan 2.1 T2V (video). Pulls now also reuse the local huggingface hub cache: a byte-exact snapshot file (diffusers/ComfyUI/`hf download` installs) is hardlinked into the models dir — zero re-download, and `blazar rm` still only removes blazar's own link, never the hub blob.
- **The sdcpp installer picks up a CUDA prebuilt the day upstream ships one.** The asset picker now takes the GPU vendor into account: an NVIDIA box prefers a CUDA zip (self-contained build; the size-optimized `cudart` variant is skipped), every other GPU rides Vulkan, and the CPU resort can no longer mislabel a CUDA zip as its fallback. Windows already publishes CUDA builds — NVIDIA Windows installs take them starting now; a future Linux CUDA release needs zero code change on our side (pinned against synthetic future assets).
...
- **Every pull lane now re-uses byte-exact files before downloading.** Content-first reuse existed only on the diffusion-component path; a row-less re-pull (store row deleted, file intact on disk) rode the main download path and re-downloaded a byte-exact 4.3 GiB DiT through `.part` — only `.part` resume existed there. The bare-leaf → collision-resolved dest → huggingface-hub-cache decision is now one shared helper consulted by every download site (gguf shards, mmproj sidecars, safetensors shards, components): a hit records the reused path on the row, advances the progress bar, and sweeps the stale `dest.part`/`.part.progress` orphans from interrupted attempts. Multi-shard pulls adopt a bare leaf only when the set is a single file, so shard-set derivation from the recorded path never strands siblings at a disambiguated name. Live-verified: the exact incident scenario now rebuilds the row and full component set in 12 s with zero network transfer (a wiremock pin additionally proves exactly one blob request across both pulls).
- **Session-bank save/restore now report the token truth, not just HTTP 200.** A bank restore could log "restored banked session" while the checkpoint actually injected zero tokens (upstream has a restored-then-empty re-prefill bug class). Both slot actions now parse the engine's own counts from the response — restore logs the injected token total and warns "continuing cold" on a zero-token 200; save warns when it banks an empty slot instead of a cached prefix (happy path stays a debug line). The counts are live-verified semantics: a 31-token save/restore cycle on this engine served the next same-prefix request with `cache_n = 23, prompt_n = 1` (96% prefill skip). The `cache_reuse` knob doc no longer claims "cross-slot prefix sharing" — `--cache-reuse N` reuses KV chunks within one child's own slot cache, per-process, never cross-replica.
...
- **`blazar search --quant <q>`: filter search results by quantization on each repo's real file set.** The Hub exposes no quant facet, so the filter runs client-side against the same quant set the QUANTS column renders (per-file GGUF tokens, repo-id bit-width markers for safetensors/MLX lanes — filter and table can never disagree). `--quant q4` matches the whole Q4 family (`q4_0`, `q4_k_m`, `q4_k_xl`, `iq4_xs`, …), `--quant Q4_K_M` that exact quant, comma-lists OR (`q4,q5`), and matching is case-insensitive; an `iq4` filter stays i-quant-only. A bare query token that is itself a quant (`blazar search qwen image 2.1 gguf q4`) is lifted out of the Hub full-text query into the filter automatically — such tokens are dead weight server-side — with a stderr notice (stdout stays clean for `--json` pipes); `--format` composes independently. Quant token extraction also widened, verified against a 4,346-file live Hub scan: double-extension layouts (`Q4_K_M.GGUF.gguf`, ~5% of scanned files) and quant-in-folder layouts (`Q4_K_M/model.gguf`) now yield tokens in both the search table and the `pull <repo>:quant` path, which shares the grammar.
...
### Changed
- **The interactive REPL chat stream is sacred: no `[profile]` lines after replies anymore.** Profile telemetry for a run (KV/ctx fit, slot autofit, load-mode choice, spec fallback) now reaches the user only through `blazar ps` (table `warn[...]` rows and `--json`) — the surfaces you consult on purpose — while pure auto-tuning decision notes (`slots auto`, `slots auto-fit`, `spec=auto` dense fallback, `load-mode mlock auto`, and the `cache_ram_mb` default meeting the adaptive 30%-of-RAM cap) moved to the daemon journal (`journalctl -u blazar`), emitted once per spawn. What stays a `blazar ps` warning is input divergence worth acting on: a user-pinned `cache_ram_mb` that got clamped, a spec draft pair that exists in the catalog but is not pulled, engines lacking flags you requested. Net effect on a default-config small-RAM box: zero warning noise, same decisions, same journal audit trail.
...
### Added
- **`blazar doctor` routing rows (2026-09-17).** Doctor now resolves every pulled model through the same lane resolver as the listings and warns per-model when nothing can serve it, quoting the exact `blazar engine install --kind <kind>` remedy; a healthy store gets one `routing` ok row naming the active mode.
- **`blazar list` shows the routed engine per model (2026-09-17).** New ENGINE column resolves through the same lane logic as `/v1/models` and `/api/tags` — the three listings can never disagree. Manual mode shows the global active engine; auto mode routes per format+policy; `-` means nothing installed can serve the model.
...
- **`blazar engine use --kind <kind>`** — switch lanes by kind, newest-first.
...
- **Wrong-lane teaching names the engine you already have (2026-09-17).** Pulling a safetensors model while llamacpp is active in manual mode used to advise `blazar engine install --kind sglang` even when sglang was installed all along — the error could not see the store. The mismatch teaching now appends the installed lane and the zero-flip remedy: `... but sglang-0.5.19 is already installed: blazar engine use sglang-0.5.19 (then restart the daemon), or let routing pick per model: blazar config set engine_routing.mode auto`.
...
- **Installer hangs forever running `blazar serve` from a comment** — dash executes backticks inside `$( )` heredocs at parse time; also wrote a corrupted unit file.
...
### Added
- **`blazar engine use --kind <kind>` (2026-09-17).** Shorthand for switching the serving lane by engine kind instead of tag — resolves the newest installed row of that kind (newest-first, the same contract rollback relies on); the plain tag form is unchanged.
...
### Changed
- **Engine retention tightened from 3 to 2 installed builds (2026-09-13).** Auto-prune (which runs after every engine install) now keeps the newest engine plus one rollback anchor (~215 MiB each); `local` and the active tag stay protected on top. Users who want deeper history still have `blazar engine use <tag>` re-download on demand.
- **Prebuilt CUDA engine channel slimmed to consumer archs + PTX forward-JIT (2026-09-13).** The overlay CI (`engine-cuda`) built fat binaries across every SASS arch (sm 61–120 per toolkit, both real+virtual per arch). Builds now emit SASS for the consumer set only (`61-real;75-real;86-real;89-real;120` on CUDA 12.8.1, `75-real;86-real;89-real;120` on 13.0) plus full SASS+PTX on the newest arch, so future GPUs JIT forward from its PTX — roughly a third of the nvcc work and a much smaller download. Datacenter archs (sm 70/80/90/100) stay reachable through the source lane (`blazar engine build cuda`). The CI link step also resolves CUDA driver-API symbols via the toolkit stubs (GPU-less runners have no `libcuda.so.1`; the stub's SONAME keeps the runtime NEEDED entry correct on real machines).

### Fixed

- **`install.sh` no longer hangs forever running `blazar serve` from a comment (2026-09-16).** The systemd-unit heredoc is collected inside a command substitution, and dash executes backticked text — and any literal dollar-parenthesis — at PARSE time, before the heredoc is even read. A unit comment reading "`blazar serve`" therefore launched the real server with its stdout wired to the capture pipe: the installer blocked reading a daemon that never exits (40+ minutes, twice, terminal blinking), and once unblocked the `sudo tee` wrote the corrupted capture (the server's startup banner spliced mid-comment) into `/etc/systemd/system/blazar.service`. The backticks are gone, the heredoc carries a NOTE forbidding both syntaxes in its body, and the repaired installer ran end-to-end clean (unit rewritten, service active, healthz green). Reproduced with a 6-line dash repro and `sh -x` on a fakesystemctl sandbox before the fix; verified clean after.
...
- Bare `blazar engine install` no longer silently defaults to `--kind mistralrs` — it prints the lane chooser (all four lanes, their model formats, and the exact install command each). `blazar engine list` now catalogs every installable-but-absent lane plus the separate whisper voice lane (tag or install command), and the engine group help cross-links voice.
- Two daemons, one lock: a session-spawned `blazar serve` holding the pidfile while the systemd unit respawns used to exit 1 in a `Restart=always` crash loop. Lock refusals now carry a `LockHeld` marker and exit the same singleton-conflict code as a port bind conflict (exit 3, already covered by the unit's `RestartPreventExitStatus`), with the remove-pidfile teaching — verified live.
- `blazar quantize` is now crash-atomic: output writes to a hidden `.part-<pid>` sibling promoted by rename only after the verify gates, so an interrupted run never leaves an orphan that blocks the next one. `blazar pull` missteps name the model verbatim (no `library/` prefix) and teach the registry's no-tag-listing reality with a `search` pointer; the upgrade banner prints only on success. `blazar tune --load` announces the model load behind its first probe row; `lora`/`session`/`search` args carry real descriptions; `blazar coreside` states the device-backed KV truth and prints `-` for unmeasurable KV instead of a fake `0M`.
...
- **`blazar run` renders thinking deltas dimmed on a real terminal.** The streaming REPL printed only `message.content` — reasoning vanished. On a TTY, thinking deltas render inline dimmed (ANSI 2m) with a single separator before the first content token; piped output omits thinking entirely so `jq`/scripts stay clean.
- **`blazar bench`/`tune` find llama-bench in any engine layout and teach the real remedy when it is absent.** The locator hardcoded the classic `engines/<tag>/llama-<tag>/` path, so per-arch slim assets (which unpack into vendor-suffixed dirs like `llama-b10948-bin-ubuntu-cuda-13.0-sm89-x64`) missed the bench binary even if present — and its error sent users down a dead end (`blazar engine update` fetches another server-only slim asset). The search now walks each llamacpp engine dir for the real layout (reusing the engine module's symlink-safe walk; active engine first, shallowest match = deterministic), only accepts a `llama-bench` with `llama-server` in the same dir (the Tuner spawns load/replica probes from the bench dir — a tool-only dir would fail confusingly later), skips mistralrs/sglang rows entirely, and the error names remedies that actually produce a bench binary: `blazar engine build cuda` (the source lane builds the full tool set) or `blazar engine local <dir>` for a full bundle. The validator's engine lanes now run a REAL switch dance when two or more server-bearing engines exist (slim boxes previously fell into a fail-all branch despite having two), and its bench/tune lanes boundary honestly — carrying the remedy text — when no llama-bench is installed, instead of failing red.
...
- `blazar list` gained a TYPE column: `gguf` (single-file GGUF), `safetensors` (HF-style directory for the mistralrs/sglang lanes), `dir?` (unexpected dir layout — inspect with `blazar show`), `missing!` (stale row whose file is gone, matching the VISION column convention).
...
- **Per-arch CUDA engine assets (~60% smaller downloads).** The prebuilt CUDA channel now publishes one slim asset per GPU architecture (`llama-bNNNN-bin-ubuntu-cuda-13.0-sm89-x64.tar.gz`, 9 assets per tag across CUDA 12.8/13.0) instead of a fat multi-arch tarball; the sm120 asset carries PTX for forward JIT on future GPUs. `blazar engine update` picks the exact asset for the local GPU's compute capability, falls back to legacy fat assets during the transition, then to the PTX asset for GPUs newer than sm120. CI wall time drops in parallel (per-arch jobs ~15-20min with ccache vs ~50min serial fat build).
...
- `blazar engine prune` — manual trigger for the engine retention policy (same keep-newest + protect-active/local logic that runs automatically after each install); useful after lowering retention or cleaning up accumulated builds.
- Installer: `BLAZAR_UNIT_MEMORY_HIGH` sets a systemd `MemoryHigh` soft ceiling on the daemon cgroup (default `85%` of RAM, recomputed by systemd at every unit start — soft reclaim/throttle only, never an OOM kill; empty string omits the line).
- **Sentinel detection codes unified to snake_case everywhere
  ...
  former, so doctor's own hint `blazar why --code ReasoningNoAnswer`
  ...
  over all nine codes.
- **`blazar engine update` no-op on source-built engines — now routes
  ...
  lane warning.
- **`blazar why` observability dead-end + actionable
  ...
  clean batch were unreachable, making doctor's `blazar why` hint a
  ...
  at the 256-record ring) plus `blazar why --flagged/--code/--model/
  --limit`; filters apply before the newest-first cut. Doctor's
  sentinel hint is now directly runnable (`blazar why --flagged`,
  plus `blazar why --code <dominant>`). ReasoningNoAnswer detail now
  ...
  (`BLAZAR_VALIDATE=1` only) and validate.py routes SIGTERM/SIGHUP/
...
- **`reasoning` config knob — llama.cpp's server-side reasoning
  ...
  default emits no flag, argv byte-identical. Completes Blazar's
...
- **`logprobs` mapped back in `/api/chat`.** Requests with
  ...
  directly).
- **`blazar doctor` flags model-dir orphans**: GGUF files in the
  ...
  `blazar import <file> --name <n>` hint — import to register or
...
- **`blazar doctor` flags config pins that mirror retired defaults**:
...
- **GPU driver preflight — the zero-touch last mile**: the Linux
  ...
  install ends with the loud chain: REBOOT, then `blazar engine
  update` — which auto-picks the newest CUDA build the driver supports
  (runtimes bundled; no CUDA toolkit). Opt out with
  `BLAZAR_AUTO_DRIVER=0` (acknowledged even when the engine bootstrap
  is off; skipped entirely under `BLAZAR_INSTALL_ENGINE=0`). The
  Windows installer is advise-only (detects NVIDIA hardware without
  `nvidia-smi`, links the driver download). `blazar doctor` gains
...
- **Profile warnings surfaced to users**: compile-time decisions
  ...
  `/api/ps` rows as `blazar_warnings`, print as `[profile] …` lines
  after a `blazar run` stream, and as `warn[<model>]` lines under the
  `ps` table. The REPL prints them once after the first turn of each
  model (re-armed on `/model` switches).
- **`run` auto-pull**: `blazar run <model>` with a model missing from
  the store now pulls it first (the exact `blazar pull` flow —
  ...
  not-found teaching error. `blazar pull` itself is unchanged.
...
- **Colon-name resolution (ollama muscle memory)**: every model-taking
  ...
  plus a flat-form teaching hint (`blazar names are flat; …-form
...
- **`ps` names the card**: `/api/ps` rows carry `blazar_device` and
...
- **Grouped top-level help**: `blazar --help` renders commands by
  ...
  Per-command help (`blazar help <cmd>`, `<cmd> --help`), error usage
  and shell completions remain clap-native.
- **`blazar why --watch`**: tails sentinel detections live (same lane
  as `blazar watch`); conflicts with a positional TRACE (loud clap
  error, not a silent precedence).
- **Refusal hint**: the local-only refusal stubs (`push`/`signin`/
  `login`/`signout`/`logout`) now also point at `blazar search` for
  GGUF discovery, not just `blazar pull`.
...
- **Prebuilt CUDA engine channel** (`engine-cuda` CI workflow + runtime
  ...
  `blazar engine update` prefers them automatically on Linux-NVIDIA
  (zero-touch; `BLAZAR_ENGINE_REPO=owner/repo` points at a fork)
  ...
  unchanged. `blazar engine install bNNNN-cuda` pins an overlay build
...
- **Bench cross-runtime prefill parity**: the ollama lane (no tokenize
  route) sized prompts by char estimate — 371 real tokens vs blazar's
  527 at the same target. Lanes now converge on the engine's own
  `prompt_eval_count`; re-measured same-token counts put blazar at or
...
- **`run` parses its flags instead of swallowing them into the
  prompt**: `trailing_var_arg` made `blazar run m 'Say ok'
...
- **Circuit breaker counts crash restarts, not churn**:
  ...
  5th and 503'd until `blazar ps --reset` (live-repro'd). Restarts
...
- **Cold-start TTFT measured**: methodology + numbers landed in README
  ...
  engine load (mmap + CUDA init); blazar-side stages (profile
  compile, health poll, admission) are milliseconds. A later 9B
  parity re-run at uncontended conditions measured blazar cold
...
- **Help branding de-ollama'd**: top-level about, help footer, and the
  ...
  `logout`) now say "blazar" instead of "ollama.com"/"ollama-grade".
...
- **Benchmark cold/idle/ctx lanes** (`scripts/bench_matrix.py`): fadvise
  ...
  runtimes), blazar `cold_ttft_ms` capture, ollama cold lane
  (`load_duration` counters, optional systemd-restart daemon-boot metric
  via `BENCH_SUDO_PASSWORD`), idle-wake lane (blazar sleep ladder vs
...
- **Harness mmproj fadvise parity** — cold probes drop the page cache on
  the model file AND its projector (a cached projector handed blazar a
...
- **`spec = "mtp"`** (opt-in): emits `--spec-type draft-mtp` for GGUFs whose
  ...
  profile-compile naming `blazar engine update`. Verified absent in the
...
- **Server-side tools passthrough** (experimental, global-only quartet
  ...
  validated prefixes passed through — blazar itself never requires docker.
...
- **dflash/dspark catalog pairs + exact-filename pulls**: the quant slot
  ...
  (`blazar pull ggml-org/Qwen3-8B-GGUF:dflash-Qwen3-8B-Q8_0.gguf`) — how
...
- **registry.ollama.ai pull lane**: `blazar pull qwen3:0.6b` (or explicit
  ...
  `BLAZAR_REGISTRY_TOKEN` attached only to the first-party registry host.
  ...
  authoritative in blazar). Digest pins verify the manifest body hash.
...
- **`docs/registry-ollama-pull.md`** — implementation spec for pulling
  ...
  resumable).
- **`cpu_ffn_n` knob** (`--n-cpu-ffn`, global + per-model overlay + `BLAZAR_CPU_FFN_N`):
...
- **CUDA dethrone guard + build advisory**: installing/updating a non-CUDA
  ...
  update landed); `blazar engine use <tag>` still switches explicitly.
  NVIDIA + Vulkan-only active + no CUDA engine installed now logs a
  one-time teaching advisory: `blazar engine build cuda` (one-time ~7 min
...
- **`slots = 0` (new default) is now blazar capacity-aware auto** instead
...
- The per-request `num_ctx` VRAM preflight and `blazar coreside` use the
...
### Removed
- `engine_pin` config key (+ `BLAZAR_ENGINE_PIN` env override): dead
  surface — written and documented but never read since engine pinning
  moved to the engine marker (`blazar engine use <tag>`). Configs
