...
- **mistral.rs prompts longer than 4096 tokens no longer return empty responses** — the scheduler batch step (`--max-num-batched-tokens`) now rides the serving context on both loaders (v0.9.3 default 4096 starved any longer prefill into a 200 with empty choices), and a child body that carries an error with no choices surfaces as a 502 instead of a silent empty 200.
