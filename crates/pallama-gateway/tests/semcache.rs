...
use pallama_core::{Config, SemanticCacheConfig};
...
async fn integration__semcache__header_off_bypasses() {
    ...
    let _: serde_json::Value = c
        ...
        .await
        .unwrap();
    // Same prompt with x-pallama-cache: off — served live, no cache_debug.
    let resp = c
        .post(format!("{}/api/chat", ts.base))
        .header("x-pallama-cache", "off")
        .json(&chat_body("hello semantic cache world"))
        .send()
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    assert!(resp.headers().get("x-pallama-cache").is_none());
    let r: serde_json::Value = resp.json().await.unwrap();
...
async fn integration__semcache__malformed_override_headers_400() {
    ...
    let c = client();
    for (h, v) in [
        ("x-pallama-cache", "maybe"),
        ("x-pallama-cache-ttl", "0"),
        ("x-pallama-cache-threshold", "1.5"),
    ] {
        let resp = c
            .post(format!("{}/api/chat", ts.base))
            .header(h, v)
...
async fn integration__semcache__metrics_surface() {
    ...
    let m = c
        ...
        .await
        .unwrap();
    assert!(m.contains("pallama_semantic_cache_misses_total 1"), "{m}");
    assert!(m.contains("pallama_semantic_cache_stores_total 1"), "{m}");
    assert!(m.contains("pallama_semantic_cache_entries 1"), "{m}");
    assert!(m.contains("pallama_semantic_cache_hits_total 0"), "{m}");
}
