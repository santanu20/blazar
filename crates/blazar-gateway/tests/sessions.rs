...
async fn integration__pin_mw__header_pins_model_full_stack() {
    let ts = start(blazar_core::Config::default()).await;
    let c = client();
    ...
    let r = c
        ...
        .header("x-blazar-session", "agent1")
        .json(&body)
        .send()
        .await
        .unwrap();
    ...
    let m = c
        ...
        .await
        .unwrap();
    assert!(m.contains("blazar_sessions_live 1"), "gauge in:\n{m}");
}
...
async fn integration__session_identity__manifest_written_tamper_blocks_restore() {
    ...
    bad.ctx += 1024;
    blazar_core::session_identity::write_manifest(&ckpt, &bad).unwrap();
    let r = c
        .post(format!("{}/api/session", ts.base))
        .json(&serde_json::json!({"model": "m1", "action": "restore", "filename": "conv1"}))
        .send()
