...
use blazar_core::{ApiKey, BlazarDirs, Config};
use blazar_gateway::router;
use blazar_gateway::state::AppState;
use blazar_runtime::engine_impl::LlamaCppEngine;
...
pub async fn start(config: Config) -> TestServer {
    ...
    write_gguf(&gguf);
    let store = blazar_core::Store::open(&dirs).unwrap();
    store
    ...
    let manifest = blazar_runtime::probe_manifest(&find_stub(), "stub").unwrap();
    let engine = LlamaCppEngine::new(manifest);
    let mut s = Supervisor::new(
        dirs.clone(),
        config.clone(),
        EventBus::default(),
        blazar_core::hardware::Hardware {
            physical_cores: 4,
            total_ram_mib: 16_000,
            gpus: vec![blazar_core::hardware::GpuInfo {
                name: "g".into(),
...
pub fn with_remote(mut cfg: Config, name: &str, url: &str) -> Config {
    cfg.remotes.push(blazar_core::Remote {
        name: name.to_string(),
        url: url.to_string(),
        key: String::new(),
    });
