...
//! 1. detections land in the ring (`blazar why` data), and
...
async fn await_record(ts: &TestServer, route: &str) -> blazar_gateway::sentinel::SentinelRecord {
    for _ in 0..200 {
...
fn codes(rec: &blazar_gateway::sentinel::SentinelRecord) -> Vec<&'static str> {
...
fn integration__sentinel__legacy_record_without_finish_deserializes() {
    ...
    let rec: blazar_gateway::sentinel::SentinelRecord =
...
async fn integration__sentinel__precheck_header_on_bare_template() {
    ...
    // x-blazar-warnings must name it BEFORE inference (header, not just why).
...
async fn integration__sentinel__ring_survives_restart_via_jsonl() {
    ...
    let sentinel = blazar_gateway::sentinel::Sentinel::new(true, 0, Some(&run_dir));
    // Persistence is driven through the public judge (records + commits).
    let ctx = blazar_gateway::sentinel::RequestCtx {
    ...
    let revived = blazar_gateway::sentinel::Sentinel::new(true, 0, Some(&run_dir));
    ...
    let tolerant = blazar_gateway::sentinel::Sentinel::new(true, 0, Some(&run_dir));
...
async fn integration__openai_num_ctx_header__restarts_at_requested_size() {
    ...
    // the X-Blazar-Num-Ctx extension header fills it with the same
...
async fn integration__sentinel__disabled_is_a_full_kill_switch() {
    ...
    let resp = c
        ...
        .unwrap();
    assert_eq!(resp.status(), 200);
    assert!(resp.headers().get("x-blazar-warnings").is_none());
