...
/// what was wrong with it, which knob fixes it. Powers `blazar why`.
...
pub async fn tags(State(state): State<Arc<AppState>>) -> Response {
    let models = match state.with_store(blazar_core::Store::list_models) {
        Some(Ok(l)) => l,
...
pub async fn show(State(state): State<Arc<AppState>>, body: Bytes) -> Response {
    ...
    let gguf = blazar_core::read_metadata_file(std::path::Path::new(&row.path)).ok();
    ...
    let mut resp = json!({
        ...
        "modelfile": format!("# blazar: plain GGUF at {}", row.path),
        ...
        "model_info": {
            ...
            "blazar.path": row.path,
            "blazar.repo": row.repo,
            "blazar.shards": row.shards,
        },
    });
    if let Some(p) = profile {
        resp["blazar_profile"] = json!({
    ...
    let rows: Vec<Value> = state
        ...
        .map(|p| {
            ...
            json!({
                ...
                "blazar_cache_hit": state.obs.hit_ratio(&p.name),
                "blazar_in_flight": p.in_flight,
    ...
    let rows: Vec<Value> = v["data"]
        ...
        .filter_map(|m| {
            ...
            Some(json!({
                ...
                "model": name,
                "size": 0,
                "blazar_state": blazar_state,
                "blazar_ctx": 0,
                "blazar_spec": "",
                "blazar_draft": null,
                "blazar_in_flight": 0,
                "blazar_endpoint": "router",
...
fn pull_stream_names(target: &str) -> Vec<String> {
    ...
    if let Ok(parsed) = blazar_runtime::hf::parse_pull_target(target) {
        names.push(blazar_runtime::hf::registry_name(&parsed.repo));
    }
    names
}
...
fn pull_event_line(names: &[String], e: &blazar_runtime::BlazarEvent) -> (String, bool) {
    use blazar_runtime::BlazarEvent::{ModelPulled, PullFailed, PullProgress};
...
/// GET /api/events — SSE of every `BlazarEvent` (dashboards).
...
fn default_think_off(row: &blazar_core::ModelRow, req: &mut Value) {
...
fn refuse_unsupported_think(row: &blazar_core::ModelRow, req: &Value) -> Option<Response> {
    ...
    let template = blazar_core::gguf::read_metadata_file(std::path::Path::new(&row.path))
        .ok()
...
/// Per-request spec-mode override (`options.spec`, `X-Blazar-Spec`,
...
pub(crate) async fn apply_spec(
    ...
    if !blazar_core::is_valid_spec_mode(want) {
...
/// POST /api/evict {"model": name} — blazar-internal (ollama has no REST
/// equivalent): unload a model now. Powers `blazar stop <model>`.
...
/// "filename", "slot": 0} — blazar-internal: slot KV-cache
...
pub async fn session(State(state): State<Arc<AppState>>, body: Bytes) -> Response {
    ...
    if action == "close" {
        let Some(session) = v["session"].as_str() else {
            return api_error(
                400,
                "missing 'session' (the x-blazar-session name to close)",
            );
        };
    ...
    if action == "erase" {
        let path = state
            .dirs
            .sessions_dir()
            .join(blazar_core::profile::path_safe(model))
            .join(filename);
        // Sibling identity manifest dies with the checkpoint (H19: no
        // orphaned metadata).
        let _ = std::fs::remove_file(blazar_core::session_identity::manifest_path(&path));
    ...
    if action == "restore" {
        ...
        match blazar_core::session_identity::read_manifest(&ckpt) {
            Some(saved) => {
                let current =
                    blazar_core::session_identity::build(&state.dirs, &state.config, &engine.name)
                        .map(|mut id| {
                            id.ctx = live_ctx;
                            id
                        });
                if let Some(cur) = current {
                    let diffs = blazar_core::session_identity::verify(&saved, &cur);
                    if !diffs.is_empty() {
                        return api_error(
                            400,
                            &format!(
        ...
        match resp {
            Ok(r) => {
                ...
                if status.is_success() && action == "save" {
                    let ckpt = ckpt_dir.join(filename);
                    match blazar_core::session_identity::build(
                        &state.dirs,
                        &state.config,
                        &engine.name,
                    )
                    .map(|mut id| {
                        ...
                        Some(id) => {
                            if let Err(e) =
                                blazar_core::session_identity::write_manifest(&ckpt, &id)
                            {
                                tracing::warn!(
                                    target: "blazar::session",
...
/// GET /metrics — merged children prometheus text + blazar_* gauges.
...
fn cache_metrics(state: &AppState, merged: &mut String) {
    ...
    if let Some(ratio) = cache_hit_ratio(cached_total, prompt_processed) {
        let _ = write!(
            merged,
            "# HELP blazar_cache_hit_ratio Reused prompt tokens / all prompt tokens seen (cached + processed), summed from live engine scrapes (authoritative)\n# TYPE blazar_cache_hit_ratio gauge\nblazar_cache_hit_ratio {ratio:.4}\n"
        );
    }
    let _ = write!(
        merged,
    ...
    let _ = write!(
        merged,
        "# HELP blazar_sessions_live Sessions with an unexpired eviction pin (x-blazar-session)\n# TYPE blazar_sessions_live gauge\nblazar_sessions_live {live_sessions}\n"
    );
    let _ = write!(
        merged,
...
pub async fn metrics(State(state): State<Arc<AppState>>) -> Response {
    ...
    // F25: router mode serves N models through ONE blazar instance —
    ...
    let _ = write!(
        merged,
        "# HELP blazar_models_loaded Models currently loaded\n# TYPE blazar_models_loaded gauge\nblazar_models_loaded {models_loaded}\n"
    );
    let _ = write!(
        merged,
    ...
    let _ = write!(
        merged,
        "# HELP blazar_audit_dropped_total Audit lines dropped (writer saturated or IO failure)\n# TYPE blazar_audit_dropped_total counter\nblazar_audit_dropped_total {}\n",
        state
            .audit_dropped
            .load(std::sync::atomic::Ordering::Relaxed)
    );
    ...
    let _ = write!(
        merged,
        "# HELP blazar_otlp_dropped_total OTLP spans dropped (channel saturated or export failure)\n# TYPE blazar_otlp_dropped_total counter\nblazar_otlp_dropped_total {}\n",
        state
            .otlp
            .dropped
            .load(std::sync::atomic::Ordering::Relaxed)
    ...
    let _ = write!(
        merged,
        "# HELP blazar_key_usage_requests Requests today per key\n# TYPE blazar_key_usage_requests counter\n"
    );
    for (name, _, req, _) in state.keys.usage_snapshot() {
        let _ = writeln!(
            merged,
            "blazar_key_usage_requests{{key=\"{}\"}} {req}",
            escape_label(&name)
        );
    }
    let _ = write!(
        merged,
        "# HELP blazar_key_usage_tokens Tokens today per key\n# TYPE blazar_key_usage_tokens counter\n"
    );
    for (name, _, _, tok) in state.keys.usage_snapshot() {
        let _ = writeln!(
            merged,
            "blazar_key_usage_tokens{{key=\"{}\"}} {tok}",
            escape_label(&name)
        );
    }
    ...
    // batch needs; flag it so operators flip spec=off under load.
    {
        ...
        let _ = write!(
            merged,
            "# HELP blazar_spec_saturation 1 = spec decoding active while requests are queued (drafting competes with the batch)\n# TYPE blazar_spec_saturation gauge\nblazar_spec_saturation {}\n",
    ...
    let _ = write!(
        merged,
        "# HELP blazar_model_prefix_heat Recency-weighted prefix heat (capacity-eviction bias)\n# TYPE blazar_model_prefix_heat gauge\n"
    );
    for p in state.sup.ps() {
        let _ = writeln!(
            merged,
            "blazar_model_prefix_heat{{model=\"{}\"}} {}",
            p.name, p.heat
        );
    }
    ...
    let _ = write!(
        merged,
        "# HELP blazar_slo_deadline_exceeded_total Requests admitted AFTER their SLO deadline expired (served late: queue burn)\n# TYPE blazar_slo_deadline_exceeded_total counter\nblazar_slo_deadline_exceeded_total {}\n",
...
mod tests {
    ...
    fn row_with_path(path: &str) -> blazar_core::ModelRow {
