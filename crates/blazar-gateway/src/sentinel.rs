...
/// `blazar why --code` hint dead-ended against the filter.
...
/// `blazar why` survives daemon restarts). Metadata only — never content.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
pub struct Sentinel {
    ...
    /// Live tail for `blazar watch` / `GET /api/watch`: every committed
...
impl Sentinel {
    ...
    pub fn template_support(&self, model: &str, gguf_path: &Path) -> Option<TemplateSupport> {
        ...
        let meta = blazar_core::read_metadata_file(gguf_path).ok()?;
    ...
    /// Ring query for `/api/why` + `blazar why`: exact trace match, or
    ...
    /// Subscribe to the live record stream (`blazar watch`).
    #[must_use]
    ...
    fn persist_record(&self, record: &SentinelRecord) {
        ...
        if let Err(e) = writeln!(p.file, "{line}") {
            tracing::debug!(target: "blazar::sentinel", "persist append: {e}");
        } else {
...
/// Resolve enforce for a request: an explicit `X-Blazar-Enforce` header
/// wins over the config default (`1`/`0`; absent = config value).
#[must_use]
pub fn enforce_enabled(config: &blazar_core::Config, headers: &axum::http::HeaderMap) -> bool {
    match headers
        .get("x-blazar-enforce")
...
/// Post-`why` filtering for `/api/why` + `blazar why`: substring model,
...
mod tests {
    ...
    fn unit__code__derive_serialization_matches_as_str() {
        ...
        // or `blazar why --code <hinted>` dead-ends.
