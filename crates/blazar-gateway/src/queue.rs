...
//! Explicit `x-blazar-deadline-ms` waiters keep strict EDF (never
...
struct Waiter {
    ...
    /// `x-blazar-deadline-ms` (strict EDF, never WFQ-delayed).
...
/// `x-blazar-deadline-ms` overrides; prefill-heavy bodies (large
...
impl PriorityQueue {
    ...
    /// (from `x-blazar-deadline-ms`) or class defaults order waiters
...
mod tests {
    ...
    async fn unit__queue__wfq_explicit_deadline_barrier_holds() {
        ...
        // `x-blazar-deadline-ms` waiter via a fresh WFQ bucket, even
