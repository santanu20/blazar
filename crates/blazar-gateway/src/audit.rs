...
pub async fn writer_task(
    ...
    if let Err(e) = std::fs::create_dir_all(&dir) {
        tracing::warn!(target: "blazar::audit", "audit dir {}: {e}", dir.display());
    ...
    while let Some(line) = rx.recv().await {
        ...
        if written >= AUDIT_ROTATE_BYTES {
            if let Err(e) = rotate(&path) {
                tracing::warn!(target: "blazar::audit", "audit rotate: {e}");
            } else {
                tracing::info!(
                    target: "blazar::audit",
        ...
        match std::fs::OpenOptions::new()
            ...
            Err(e) => {
                dropped.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                tracing::warn!(target: "blazar::audit", "audit append: {e}");
            }
        }
    }
}
