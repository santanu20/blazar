//! `pallama upgrade` e2e against a fake GitHub Releases server.
//!
...
fn preferred_asset(tag: &str) -> String {
    pallama_runtime::upgrade::preferred_assets(tag)
        .into_iter()
        .next()
...
fn e2e__upgrade_dry_run_resolves_and_verifies() {
    let base = fake_release("v0.1.1", b"FAKE-PALLAMA-v0.1.1\n", None);
    Command::cargo_bin("pallama")
        .unwrap()
        .env("PALLAMA_INSTALL_BASE_URL", &base)
        .env("PALLAMA_REPO", "test/pallama")
...
fn e2e__upgrade_replaces_a_copy_of_the_binary_atomically() {
    let payload = b"FAKE-PALLAMA-v0.1.2-REPLACED\n";
    let base = fake_release("v0.1.2", payload, None);

    // Run the upgrade from a COPY so the cargo artifact is never touched.
    let tmp = tempfile::tempdir().unwrap();
    let exe_copy = tmp.path().join("pallama");
    std::fs::copy(env!("CARGO_BIN_EXE_pallama"), &exe_copy).unwrap();
