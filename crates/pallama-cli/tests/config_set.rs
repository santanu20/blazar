//! `pallama config set` regression pins.
//!
...
//!
//! Isolation: `XDG_CONFIG_HOME` -> tempdir (`PallamaDirs::from_env` honors XDG).
...
fn pallama(cfg_root: &Path) -> Command {
    let mut c = Command::cargo_bin("pallama").unwrap();
...
fn seed(cfg_root: &Path) {
    std::fs::create_dir_all(cfg_root.join("pallama")).unwrap();
    std::fs::write(
        cfg_root.join("pallama/config.toml"),
...
fn e2e__config_set_invalid_value_rejected_file_unchanged() {
    ...
    let cfg_root = tmp.path();
    seed(cfg_root);
    let before = std::fs::read_to_string(cfg_root.join("pallama/config.toml")).unwrap();
    ...
    let after = std::fs::read_to_string(cfg_root.join("pallama/config.toml")).unwrap();
