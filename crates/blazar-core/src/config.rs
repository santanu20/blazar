...
pub struct Config {
    ...
    /// `"blazar=trace,blazar::engine=debug"`. `None` = built-in default
    ...
    /// release; `blazar engine update`/`blazar upgrade` then move to it.
    #[serde(default)]
    ...
    /// podman-container:<id> | ssh:<target>. Passthrough only — blazar
    ...
    /// (`blazar keys add`). The legacy flat `api_keys` list is gone.
    #[serde(default)]
    pub keys: Vec<ApiKey>,
    ...
    /// Server slots (`-np`). 0 = blazar auto (default): capacity-aware
    ...
    #[serde(default)]
    pub agent: bool,
    /// Enable session checkpoints (`--slot-save-path`): `blazar session`
    ...
    /// eviction after a request carrying `x-blazar-session: <name>`.
    /// Each such request refreshes the window; `POST /api/session
    /// {"action":"close","session":name}` releases immediately, and
    /// `blazar stop` / `/api/stop` always wins. 0 disables pinning
    ...
    /// working). The `BLAZAR_CAPABILITY_REGISTRY` env var overrides
    ...
    /// must stay off (per-request `x-blazar-cache: off` escape hatch).
    #[serde(default)]
    ...
    /// from `blazar doctor`'s device list. Empty = engine auto.
    #[serde(default)]
    ...
    /// `ps`, and the log — `blazar engine update` stays a human action.
    ...
    /// there). Per-request `X-Blazar-Enforce: 1|0` overrides.
    #[serde(default)]
    ...
    /// OTLP service.name tag (default "blazar").
    #[serde(default)]
    ...
    /// default true; requires --cache-ram, which Blazar sets). Disable
...
/// `options.spec` / `X-Blazar-Spec` override — one list, one teaching
...
/// value this Blazar build does not know still reaches a newer sglang
/// that supports it).
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SglangTuning {
    ...
    /// `usage.prompt_tokens_details`. Blazar-managed sglang children
...
/// One external OpenAI-compatible server (another blazar, vLLM, MLX
...
#[serde(deny_unknown_fields)]
pub struct ApiKey {
    /// Human label (`blazar keys add NAME`).
    pub name: String,
...
/// (`blazar config set warm_peg.sglang false`). Engines outside the
...
pub enum RoutingMode {
    ...
    /// Blazar) — opt back in with `mode = "manual"`.
    Manual,
...
pub struct SemanticCacheConfig {
    ...
    /// Entry lifetime in seconds (per-request `x-blazar-cache-ttl`
    /// overrides, 1..=86400).
    #[serde(default = "default_semantic_ttl_secs")]
    pub ttl_secs: u64,
    /// Cosine similarity gate in (0, 1] (per-request
    /// `x-blazar-cache-threshold` overrides).
...
pub fn persist_config(path: &std::path::Path, body: &str) -> CoreResult<()> {
    ...
    // three parallel `blazar upgrade` CLIs creating the config on a
...
impl Config {
    ...
    pub fn load(dirs: &BlazarDirs) -> CoreResult<Self> {
    ...
    pub fn from_toml(raw: &str) -> CoreResult<Self> {
        ...
        // `blazar migrate` persists the canonical form; installers and
        ...
        let raw = if migrating {
            ...
            tracing::warn!(
                ...
                "legacy api_keys migrated to [[keys]] ({} key(s), in memory) — persist with `blazar migrate`",
                cfg.keys.len()
            );
    ...
    fn strip_removed_knobs(raw: &str) -> CoreResult<String> {
        ...
        if !fresh.is_empty() {
            tracing::warn!(
                target: "blazar::config",
                "removed knob(s) ignored: {} — deleted in this version; persist the canonical form with `blazar migrate`",
                fresh.join(", ")
            );
    ...
    /// for `blazar doctor` to surface as a WARN — a deliberate pin is
    ...
    fn validate_keys(&self) -> CoreResult<()> {
        for (i, k) in self.keys.iter().enumerate() {
            ...
            if k.key.trim().is_empty() {
                return Err(CoreError::Config(format!(
                    "keys[{}]: key secret must not be empty (generate with `blazar keys add {}`)",
                    i, k.name
                )));
            }
        }
    ...
    /// Apply documented `BLAZAR_*` env overrides on top of file values.
    ...
    pub fn with_env_overrides(&self) -> CoreResult<Self> {
        ...
        if let Some(v) = env("BLAZAR_HOST") {
            cfg.host = v;
        }
        if let Some(v) = env("BLAZAR_PORT") {
            cfg.port = parse_u16("BLAZAR_PORT", &v)?;
        }
        if let Some(v) = env("BLAZAR_DEFAULT_CTX") {
            cfg.default_ctx = parse_u32("BLAZAR_DEFAULT_CTX", &v)?;
        }
        if let Some(v) = env("BLAZAR_IDLE_SLEEP_SECS") {
            cfg.idle_sleep_secs = parse_u64("BLAZAR_IDLE_SLEEP_SECS", &v)?;
        }
        ...
        if let Some(v) = env("BLAZAR_CHILD_TRANSPORT") {
            cfg.child_transport = v;
        }
        if let Some(v) = env("BLAZAR_ENGINE_ASSET") {
            cfg.engine_asset = v;
        }
        ...
        if let Some(v) = env("BLAZAR_LAZY_MODE") {
            cfg.lazy_mode = v;
        }
        if let Some(v) = env("BLAZAR_SERVER_TOOLS") {
            cfg.server_tools = Some(v);
        }
        if let Some(v) = env("BLAZAR_SERVER_TOOLS_RUNTIME") {
            cfg.server_tools_runtime = Some(v);
        }
        if let Some(v) = env("BLAZAR_MCP_SERVERS_CONFIG") {
            cfg.mcp_servers_config = Some(v);
        }
        if let Some(v) = env("BLAZAR_MCP_SERVERS_JSON") {
            cfg.mcp_servers_json = Some(v);
        }
        if let Some(v) = env("BLAZAR_CACHE_REUSE") {
            cfg.cache_reuse = parse_u32("BLAZAR_CACHE_REUSE", &v)?;
        }
        if let Some(v) = env("BLAZAR_KEYS") {
        ...
        if let Some(v) = env("BLAZAR_RPC_SERVERS") {
            cfg.rpc_servers = v;
        }
        if let Some(v) = env("BLAZAR_CACHE_RAM_MB") {
            cfg.cache_ram_mb = parse_i64("BLAZAR_CACHE_RAM_MB", &v)?;
        }
        if let Some(v) = env("BLAZAR_CPU_RANGE") {
            cfg.cpu_range = v;
        }
        if let Some(v) = env("BLAZAR_POLL") {
            cfg.poll = parse_u32("BLAZAR_POLL", &v)?;
        }
        if let Some(v) = env("BLAZAR_REASONING_FORMAT") {
            cfg.reasoning_format = v;
        }
        if let Some(v) = env("BLAZAR_SLOTS") {
            cfg.slots = parse_u32("BLAZAR_SLOTS", &v)?;
        }
        if let Some(v) = env("BLAZAR_CACHE_TYPE") {
            cfg.cache_type = v;
        }
        ...
        if let Some(v) = env("BLAZAR_CPU_MOE_N") {
            cfg.cpu_moe_n = parse_i32("BLAZAR_CPU_MOE_N", &v)?;
        }
        if let Some(v) = env("BLAZAR_CPU_FFN_N") {
            cfg.cpu_ffn_n = parse_i32("BLAZAR_CPU_FFN_N", &v)?;
        }
        if let Some(v) = env("BLAZAR_OVERRIDE_TENSOR") {
        ...
        if let Some(v) = env("BLAZAR_SLOT_PROMPT_SIMILARITY") {
            cfg.slot_prompt_similarity = parse_f64("BLAZAR_SLOT_PROMPT_SIMILARITY", &v)?;
        }
        if let Some(v) = env("BLAZAR_SENTINEL") {
            cfg.sentinel = parse_bool("BLAZAR_SENTINEL", &v)?;
        }
        if let Some(v) = env("BLAZAR_SENTINEL_STALL_SECS") {
            cfg.sentinel_stall_secs = parse_u64("BLAZAR_SENTINEL_STALL_SECS", &v)?;
        }
        if let Some(v) = env("BLAZAR_SENTINEL_ENFORCE") {
            cfg.sentinel_enforce = parse_bool("BLAZAR_SENTINEL_ENFORCE", &v)?;
        }
        if let Some(v) = env("BLAZAR_AUDIT_LOG") {
            cfg.audit_log = parse_bool("BLAZAR_AUDIT_LOG", &v)?;
        }
        cfg.validate()?;
        Ok(cfg)
    }

    /// Later-wave env knobs (`BLAZAR_AGENT` ..= `BLAZAR_ENGINE_CHECK_SECS`),
    /// split out only to keep `with_env_overrides` under the line budget.
    fn apply_wave_env_overrides(cfg: &mut Self) -> CoreResult<()> {
        let env = |k: &str| std::env::var(k).ok().filter(|v| !v.is_empty());
        if let Some(v) = env("BLAZAR_AGENT") {
            cfg.agent = parse_bool("BLAZAR_AGENT", &v)?;
        }
        if let Some(v) = env("BLAZAR_SESSIONS") {
            cfg.sessions = parse_bool("BLAZAR_SESSIONS", &v)?;
        }
        if let Some(v) = env("BLAZAR_ROUTER") {
            cfg.router = parse_bool("BLAZAR_ROUTER", &v)?;
        }
        if let Some(v) = env("BLAZAR_ROUTER_MAX_MODELS") {
            cfg.router_max_models = parse_u32("BLAZAR_ROUTER_MAX_MODELS", &v)?;
        }
        if let Some(v) = env("BLAZAR_DEVICES") {
            cfg.devices = v
                .split(',')
                .map(str::trim)
                .filter(|s| !s.is_empty())
        ...
        if let Some(v) = env("BLAZAR_ENGINE_CHECK_SECS") {
            cfg.engine_check_secs = parse_u64("BLAZAR_ENGINE_CHECK_SECS", &v)?;
        }
        Ok(())
    }
}
...
mod tests {
    ...
    fn unit__lookup_cache__validation_requires_existing_file() {
        ...
        let d = std::env::temp_dir().join("blazar-lc-test-dyn.bin");
    ...
    fn unit__full_manifest_roundtrip_toml__stable() {
        ...
        let lks = std::env::temp_dir().join("blazar-gold-lks.bin");
        let lkd = std::env::temp_dir().join("blazar-gold-lkd.bin");
...
"#;
    ...
    fn unit__load_existing_file__values_honored() {
        ...
        // parallel env-override tests can't leak BLAZAR_PORT into us.
        let _g = env_lock();
        let tmp = tempfile::tempdir().unwrap();
        let dirs = BlazarDirs {
            config_dir: tmp.path().join("cfg"),
    ...
    fn unit__log_level__absent_and_present() {
        ...
        let present =
            Config::from_toml("port = 9999\nlog_level = \"blazar=trace,blazar::engine=debug\"\n")
                .unwrap();
        assert_eq!(
            present.log_level.as_deref(),
            Some("blazar=trace,blazar::engine=debug")
        );
    }
    ...
    fn unit__overlay_chat_template__xor_and_missing_file_rejected() {
        ...
        let tpl = std::env::temp_dir().join("blazar-tpl-test.j2");
...
mod persist_tests {
    ...
    fn unit__persist_config__concurrent_writers_all_succeed() {
        ...
        // parallel `blazar upgrade` CLIs on a fresh HOME crashed one CLI
