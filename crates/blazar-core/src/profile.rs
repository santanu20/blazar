...
pub struct ProfileInput<'a> {
    ...
    /// Diffusion component set (sdcpp lane): VAE weights, text encoder,
    /// and optional vision encoder for image edits. The sdcpp argv
    /// builder emits `--vae`/`--llm`/`--llm_vision` from these; `None`
    /// on every text model (the routing gate keys on exactly that).
    pub vae_path: Option<&'a str>,
    pub llm_path: Option<&'a str>,
    pub llm_vision_path: Option<&'a str>,
    ...
    /// Blazar data dir (base for speccache/ + sessions/ paths).
    pub data_dir: &'a str,
...
pub fn compile(input: &ProfileInput<'_>, tuning: &TuningOverrides) -> Result<Profile, String> {
        ...
        ModelMeta::Hf(h) => {
            return Err(format!(
                "model {} is a safetensors directory (arch {}) — llamacpp/mistralrs engines \
                 consume GGUF files; serve it on the sglang engine: `blazar engine install \
                 --kind sglang`, `blazar engine use <sglang-tag>` (see `blazar engine list`), \
            ...
            if clamped < floor_mib && floor_mib <= ram_guard {
                ...
                warnings.push(format!(
                    "cache-ram budget {clamped} MiB cannot cover weights {weights_mib} MiB \
                     ...
                     — set kv_unified = false, quant down (blazar fit), or raise the budget; \
    ...
    // --- 9. slots (resolved at rule 2a: `slots = 0` blazar-auto sizes
    ...
    if !devices.is_empty() {
        if !input.supported_flags.contains("--device") {
            return Err(format!(
                "devices set but engine {} lacks --device; run: blazar engine update",
                input.engine_tag
            ));
        }
        for dev in &devices {
    ...
    if config.main_gpu >= 0 {
        if input.supported_flags.contains("--main-gpu") {
            ...
            warnings.push(format!(
                "main_gpu set but engine {} lacks --main-gpu; run: blazar engine update",
                input.engine_tag
            ));
        }
    }
    if !config.split_mode.is_empty() {
        if input.supported_flags.contains("--split-mode") {
            ...
            warnings.push(format!(
                "split_mode set but engine {} lacks --split-mode; run: blazar engine update",
                input.engine_tag
            ));
        }
    }
        ...
        if input.supported_flags.contains("--tensor-split") {
            ...
            warnings.push(format!(
                "auto tensor-split {ratios}: weights+KV exceed the best single card's free \
                 ...
                 (blazar fit) or kv quantization may serve faster. Pin `tensor_split` \
                 to silence"
            ));
        } else {
            warnings.push(format!(
                "auto tensor-split planned but engine {} lacks --tensor-split; run: blazar engine update",
                input.engine_tag
            ));
        }
    }
    ...
    if input.overlay.late_chunking == Some(true) {
            ...
            && input.supported_flags.contains("--pooling")
        {
            ...
            warnings.push(format!(
                "late_chunking = true but engine {} lacks --embeddings/--pooling; run: blazar engine update",
                input.engine_tag
            ));
        }
        ...
        if input.supported_flags.contains("--embeddings") {
            ...
            warnings.push(format!(
                "embedding model detected but engine {} lacks --embeddings; run: blazar engine update",
                input.engine_tag
            ));
        }
    }
                ...
                && input.spec_types.iter().any(|t| t == "draft-mtp")
            {
                ...
                warnings.push(format!(
                    "spec auto: GGUF carries an MTP head but engine {} lacks \
                     ...
                     one exists); run: blazar engine update",
                    input.engine_tag
                ));
                false
            }
            ...
            || !input.spec_types.iter().any(|t| t == "draft-mtp")
        {
            return Err(format!(
                "spec = \"mtp\" needs an engine advertising draft-mtp; \
                 engine {} does not — run: blazar engine update",
                input.engine_tag
            ));
        }
        ...
        match crate::catalog::spec_pair_for_typed(input.model_name, spec_val) {
            Some(pair) => {
                if let Some(draft) = input.draft_path {
                    ...
                    return Err(format!(
                        "spec={spec_mode} for {} but the draft model is not pulled; run: blazar pull {}",
    ...
    // --- 15. session checkpoints (`blazar session save/restore`).
    ...
    if !missing.is_empty() {
        return Err(format!(
            "engine {} does not support: {}; try `blazar engine update` or `blazar engine use <tag>`",
            input.engine_tag,
            missing.join(", ")
        ));
    }
...
/// blazar cannot see ahead of spawn. Measured on an 8 GiB card
...
/// clamp as llama-server (so `blazar ps` and KV planning share one
...
///   co-resident lane blazar routes beside it — sizing KV to serving
...
fn compile_mistralrs(
        ...
        if input.supported_flags.contains("--pa-memory-fraction") {
            ...
            warnings.push(
                "config mistralrs_pa_memory_fraction set but this engine lacks \
                 --pa-memory-fraction; flag skipped (blazar engine install updates it)"
                    .into(),
            );
        }
    }
            ...
            if input.supported_flags.contains("--paged-attn") {
                ...
                warnings.push(
                    "config mistralrs_paged_attn set but this engine lacks \
                     --paged-attn; flag skipped (blazar engine install updates it)"
                        .into(),
                );
            }
        }
    ...
    // nothing for a whole ISQ A/B run), and blazar-owned flags refuse
    ...
    if let Some(extra) = &input.overlay.extra_args {
        ...
        for tok in extra {
            ...
            if RESERVED.contains(&tok.as_str()) {
                return Err(format!(
                    "extra_args {tok} is reserved — blazar owns it on the mistralrs \
...
/// server binds `--listen-ip`/`--listen-port` only AFTER the component
/// set loads (connection-refused during poll is normal loading, not a
/// crash); `--diffusion-model` takes the `DiT` GGUF, `--vae` the VAE
/// safetensors, `--llm` the text-encoder GGUF (streamed from system RAM
...
fn compile_sdcpp(input: &ProfileInput<'_>, tuning: &TuningOverrides) -> Result<Profile, String> {
    let (vae, llm) = sdcpp_components(input)?;
    ...
    if let Some(vision) = input.llm_vision_path {
        push_gated(
            input,
            &mut argv,
            &mut warnings,
            "model_overrides.llm_vision_path",
            "--llm_vision",
            &[vision.to_string()],
        );
    }
    ...
    // arm): weights(DiT)+VAE beyond three quarters of VRAM means the
    // load would OOM — sd-server's --offload-to-cpu keeps params in
    // system RAM and streams compute, the proven Q4-DiT+8B-TE-on-8GiB
    // posture. A VRAM-less box is CPU-only by construction.
    let vram_bytes = capacity_bytes(input.hardware);
    let vae_bytes = std::fs::metadata(vae).map_or(0, |m| m.len());
    let resident = input.model_bytes.saturating_add(vae_bytes);
...
/// is unservable alone. Missing paths in the store row mean either a
/// family-blinded pull (older row) or deleted files — both repair the
/// same way: re-pull.
fn sdcpp_components<'a>(input: &'a ProfileInput<'_>) -> Result<(&'a str, &'a str), String> {
    let (Some(vae), Some(llm)) = (input.vae_path, input.llm_path) else {
        return Err(format!(
    ...
    for (label, path) in [("VAE", vae), ("text encoder", llm)] {
        if !std::path::Path::new(path).is_file() {
            return Err(format!(
                "{label} file for {} is missing at {path} — the store row is stale; \
...
pub fn unified_ctx_verdict(
    ...
    UnifiedCtxVerdict::Refuse(format!(
        "pinned num_ctx {ctx} cannot fit the GPU: weights {weights_mib} MiB + KV {kv_mib} \
         ...
         — lower num_ctx, cache_type = \"q8_0\" (halves KV), or a smaller quant (blazar fit)",
...
/// Resolve the slot count and the TOTAL ctx to emit. `slots = 0` (blazar
...
/// True for every self-drafting n-gram spec mode ("ngram" is blazar
...
fn push_gated(
    ...
    if !input.supported_flags.contains(flag) {
        warnings.push(format!(
            "{key} skipped: engine {} lacks {flag}; run: blazar engine update",
            input.engine_tag
        ));
        return;
    }
...
/// long-flag names without dashes, booleans as bare `true`. Blazar
...
mod tests {
    ...
    fn draft_file(tag: &str) -> String {
        let p =
            std::env::temp_dir().join(format!("blazar-draft-{tag}-{}.gguf", std::process::id()));
    ...
    fn unit__spec_mtp__engine_lacking_draft_mtp__hard_error() {
        ...
        .unwrap_err();
        assert!(
            err.contains("draft-mtp") && err.contains("blazar engine update"),
            "{err}"
        );
    }
    ...
    fn unit__spec_eagle3__engine_lacking__hard_error() {
        ...
        .unwrap_err();
        assert!(
            err.contains("draft-eagle3") && err.contains("blazar engine update"),
            "{err}"
        );
    }
    ...
    fn unit__spec_dflash__engine_lacking__hard_error() {
        ...
        .unwrap_err();
        assert!(
            err.contains("draft-dflash") && err.contains("blazar engine update"),
            "{err}"
        );
    }
    ...
    fn unit__spec_auto__mtp_engine_lacks_draft_mtp__warn_and_no_mtp_args() {
        ...
        assert!(
            p.warnings
                .iter()
                .any(|w| w.contains("lacks") && w.contains("blazar engine update")),
            "{:?}",
            p.warnings
        );
    }
    ...
    fn unit__spec_auto_pair__engine_lacking_spec_type__warn_and_dense() {
        ...
        assert!(
            p.warnings
                .iter()
                .any(|w| w.contains("lacks it") && w.contains("blazar engine update")),
    ...
    fn unit__compile__stale_draft_row__hard_error() {
        ...
        let missing =
            std::env::temp_dir().join(format!("blazar-draft-gone-{}.gguf", std::process::id()));
    ...
    fn unit__multi_gpu_split__emitted_and_engine_gated() {
        ...
        assert_eq!(
            p2.warnings
                .iter()
                .filter(|w| w.contains("blazar engine update"))
                .count(),
            3
        );
    }
    ...
    fn unit__spec_auto__draft_present_and_missing() {
        ...
        let p = compile(&inp, &TuningOverrides::default()).unwrap();
        assert!(p.warnings.iter().any(
            |w| w.contains("not pulled") && w.contains("blazar pull ggml-org/Qwen3-0.6B-GGUF")
        ));
        assert!(!p.argv.contains(&"--spec-type".to_string()));
    ...
    fn unit__slots__overlay_shadows_global() {
        ...
        // No hint, no overlay, default slots=0 (blazar auto): capacity
    ...
    /// unknown flags fail with the manifest pointer, blazar-owned
    ...
    fn unit__mistralrs_extra_args__strict_passthrough_and_refusals() {
        ...
        assert!(
            err.contains("reserved") && err.contains("blazar owns it"),
    ...
    fn unit__auto_slots__vulkan_class_with_projector_caps_conservatively() {
        ...
        let mmproj = std::env::temp_dir().join("blazar-test-mmproj.gguf");
    ...
    fn unit__auto_slots__scoped_hardware_full_census_still_caps() {
        ...
        let mmproj = std::env::temp_dir().join("blazar-test-mmproj-scoped.gguf");
        std::fs::write(&mmproj, vec![0u8; 1024]).unwrap();
    ...
    fn unit__compile_sdcpp__component_set_listen_pair_and_offload_ladder() {
        ...
        inp.vae_path = Some(vae.to_str().unwrap());
        inp.llm_path = Some(llm.to_str().unwrap());
        let p = compile(&inp, &TuningOverrides::default()).unwrap();
        let at = |flag: &str| {
            p.argv
                .iter()
        ...
        let mut inp2 = input(&g, &hw_tight, &cfg, &empty);
        inp2.engine_kind = crate::engine_kind::EngineKind::SdCpp;
        inp2.vae_path = Some(vae.to_str().unwrap());
        inp2.llm_path = Some(llm.to_str().unwrap());
        let p2 = compile(&inp2, &TuningOverrides::default()).unwrap();
        ...
        inp3.vae_path = Some(vae.to_str().unwrap());
        inp3.llm_path = Some(llm.to_str().unwrap());
        let p3 = compile(&inp3, &TuningOverrides::default()).unwrap();
    ...
    fn unit__compile_sdcpp__missing_or_stale_component_set_teaches_repull() {
        ...
        inp.vae_path = Some("/nonexistent/vae.safetensors");
        inp.llm_path = Some("/nonexistent/te.gguf");
        let err = compile(&inp, &TuningOverrides::default()).unwrap_err();
        assert!(
    ...
    fn unit__sglang__cache_report_default_on_and_opt_out() {
        ...
        // cache-hit surfaces — blazar-managed children carry it by
