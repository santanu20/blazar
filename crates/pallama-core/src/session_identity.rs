...
#[must_use]
pub fn build(dirs: &PallamaDirs, config: &Config, model: &str) -> Option<SessionIdentity> {
    ...
    Some(SessionIdentity {
        pallama_version: env!("CARGO_PKG_VERSION").to_string(),
...
pub fn verify(saved: &SessionIdentity, current: &SessionIdentity) -> Vec<String> {
    ...
    if saved.pallama_version != current.pallama_version {
        push(
            "pallama_version",
            saved.pallama_version.clone(),
            current.pallama_version.clone(),
        );
    }
...
mod tests {
    ...
    fn unit__session_identity__verify_match_and_tamper_each_field() {
        ...
        let mut t = ident();
        t.pallama_version = "0.5.0".into();
        assert!(verify(&base, &t)
            .iter()
            .any(|d| d.contains("pallama_version")));
        let mut t = ident();
    ...
    fn integration__session_identity__build_reads_store_and_config() {
        ...
        assert_eq!(id.pallama_version, env!("CARGO_PKG_VERSION"));
    }
}
