...
fn setup(model: &str) -> (tempfile::TempDir, PallamaDirs) {
...
fn supervisor(dirs: &PallamaDirs, replicas: Option<u32>) -> Arc<Supervisor> {
...
async fn integration__evict_model__kills_all_replicas() {
    ...
    for pid in [pid_a, pid_b] {
        assert!(
            !pallama_runtime::process_alive_by_pid(pid),
