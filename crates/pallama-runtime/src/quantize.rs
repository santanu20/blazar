...
pub fn find_perplexity_bin(dirs: &PallamaDirs) -> Result<PathBuf> {
    let store = Store::open(dirs)?;
    let engines = store.list_engines()?;
    engines
        .iter()
        ...
        .ok_or_else(|| {
            anyhow!(
                "no llama-perplexity found in any installed engine; run `pallama engine update`"
            )
        })
}
...
pub fn write_verify_probe(dirs: &PallamaDirs) -> Result<PathBuf> {
    std::fs::create_dir_all(&dirs.data_dir)
        .with_context(|| format!("create {}", dirs.data_dir.display()))?;
    let path = dirs.data_dir.join("pallama-verify-probe.txt");
