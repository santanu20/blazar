...
fn manager(dirs: &BlazarDirs) -> EngineManager {
    EngineManager {
        dirs: dirs.clone(),
        gh: blazar_runtime::engine::gh::GhClient::new(None).unwrap(),
...
async fn integration__build_cpu__installs_probes_activates() {
    ...
    let m: blazar_runtime::Manifest = serde_json::from_str(&row.manifest).unwrap();
