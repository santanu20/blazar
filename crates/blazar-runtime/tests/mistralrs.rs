...
use blazar_runtime::engine::manifest::Manifest;
use blazar_runtime::engine::{EngineManager, LOCAL_TAG};
use blazar_runtime::engine_impl::Engine as _;
use blazar_runtime::engine_impl::{mistralrs_argv, MistralRsEngine};
...
async fn integration__install_mistralrs__missing_asset_error_lists_wanted() {
    ...
    let store = blazar_core::store::Store::open(&dirs).unwrap();
...
// hard error, '' override = rejected). Blazar's flat shared models dir
