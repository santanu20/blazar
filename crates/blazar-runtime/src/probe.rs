...
use blazar_core::{GpuInfo, Hardware};
...
/// blazar has spawned any child of its own. Advisory only — blazar
...
mod tests {
    ...
    fn unit__parse_gpu_tenants__csv_shapes_and_unit_suffix() {
        let t = parse_gpu_tenants(
            "1588925, /usr/local/bin/blazar, 2314\n\
             1637391, ollama, 1858\n",
        );
        assert_eq!(t.len(), 2);
        assert_eq!(t[0].pid, 1_588_925);
        assert_eq!(t[0].process_name, "/usr/local/bin/blazar");
    ...
    fn unit__probe_output__missing_binary_is_none_not_panic() {
        let mut cmd = std::process::Command::new("/nonexistent/blazar-probe-binary");
