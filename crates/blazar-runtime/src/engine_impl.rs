//! The `Engine` abstraction: everything Blazar needs from a serving
...
///   "loaded")
/// - child binds `0.0.0.0` by default and has NO auth; Blazar forces
...
pub struct MistralRsEngine {
    ...
    /// whole directory; Blazar's flat shared models dir would feed it
...
/// blazar-core next to `compile_mistralrs` — one source of truth.
fn mistralrs_tuning_passthrough(
    dialect: &[String],
    flags: &std::collections::BTreeSet<String>,
    out: &mut Vec<String>,
) {
    use blazar_core::profile::{MISTRALRS_TUNING_BOOL_FLAGS, MISTRALRS_TUNING_VALUE_FLAGS};
    let mut i = 0;
    while i < dialect.len() {
    ...
    loop {
        ...
        match reader.read_until(b'\n', &mut raw).await {
            ...
            Ok(_) => {
                ...
                tracing::debug!(target: "blazar::engine", stream, "{line}");
...
mod tests {
    ...
    fn unit__mistralrs_argv__hf_directory_rides_model_id() {
        ...
        let tmp = std::env::temp_dir().join(format!("blazar-mistralrs-hf-{}", std::process::id()));
    ...
    fn unit__mistralrs_staging__hf_directory_bypasses_shard_view() {
        ...
        // nonexistent path.
        let tmp =
            std::env::temp_dir().join(format!("blazar-mistralrs-stage-{}", std::process::id()));
