...
use blazar_core::hardware::Hardware;
use blazar_core::profile::{self, Endpoint, Profile, ProfileInput, TuningOverrides};
use blazar_core::store::{ProfileRow, Store};
use blazar_core::{BlazarDirs, ModelOverride};
...
/// lack llama-bench entirely; no external fallback paths — blazar is
/// self-contained, and the error names remedies that actually produce
/// a bench binary.
pub fn find_bench_bin(dirs: &BlazarDirs) -> Result<PathBuf> {
    ...
    if llamacpp.is_empty() {
        return Err(anyhow!(
            "no engine installed; run `blazar engine update` (prebuilt \
             per-arch CUDA assets ship llama-server only — `blazar engine \
    ...
    Err(anyhow!(
        "no llama-bench found in any installed engine. Prebuilt per-arch \
         CUDA assets ship llama-server only; `blazar engine build cuda` \
         builds the full tool set (incl. llama-bench), or register a full \
         bundle: `blazar engine local <dir>`"
    ))
}
...
impl Tuner<'_> {
    ...
    /// default: cache reuse disabled); Blazar's shipped default is 256, so
    ...
    /// Plain `blazar bench <model>`: one deterministic profile, no grid.
    ...
    /// `blazar tune <model> --search`: grid {ctx, kv-quant, threads} and
...
mod tests {
    ...
    fn test_dirs() -> (tempfile::TempDir, BlazarDirs) {
    ...
    fn unit__find_bench_bin__slim_llamacpp_store_teaches_build_cuda() {
        ...
        assert!(err.contains("blazar engine build cuda"), "got: {err}");
        assert!(err.contains("blazar engine local"), "got: {err}");
        drop(store);
        drop(tmp);
    }
    ...
    fn unit__find_bench_bin__empty_store_teaches_engine_update() {
        ...
        assert!(err.contains("blazar engine update"), "got: {err}");
        drop(tmp);
    }
