...
enum Cmd {
    ...
    /// (e.g. `blazar launch claude` / `blazar launch dsh`)
    Launch {
        ...
        #[arg(long)]
        warm: Option<String>,
        /// API key to hand the CLI (`BLAZAR_KEY` env or a key name here)
        #[arg(long)]
        key: Option<String>,
    },
    /// blazar account sign-in — refused: no cloud accounts by design
    Signin,
    /// blazar account login — refused: no cloud accounts by design
    Login,
    /// blazar account sign-out — refused: no cloud accounts by design
    Signout,
    /// blazar account logout — refused: no cloud accounts by design
    Logout,
    ...
    /// stdout — `blazar completions bash > .../blazar.bash`
    ...
    /// `blazar whisper --install` / `--pull` / `--list`.
    Engine {
        #[command(subcommand)]
        cmd: EngineCmd,
    },
    ...
    Search {
        ...
        /// the llama.cpp lane.
        /// Works in any position: blazar search minicpm --format mlx
        #[arg(
            long,
            value_name = "FORMAT",
            default_value = "any",
        ...
        /// themselves quants (`blazar search qwen gguf q4`) are lifted
    ...
    /// Self-update the blazar binary from GitHub Releases (sha256-verified)
    Upgrade {
    ...
    Why {
        /// Trace id from the x-blazar-trace-id response header
        trace: Option<String>,
...
enum EngineCmd {
    ...
    /// Activate an installed tag (see `blazar engine list` for tags)
    ///
    /// Applies to children spawned after the switch — restart the
    /// daemon (`systemctl restart blazar`, or `blazar stop` +
    /// `blazar serve`) so running instances move to the new engine.
    /// GGUF models need a llama.cpp tag active; safetensors (HF-style
    /// directory) models need mistral.rs or sglang active.
    Use {
        /// Exact engine tag (blazar engine list). Mutually exclusive
        /// with --kind.
        tag: Option<String>,
    ...
    /// active tag — `blazar engine use` another first. Reports the
    /// reclaimed bytes.
    Rm { tag: String },
    ...
    Install {
        ...
        kind: Option<String>,
        tag: Option<String>,
        /// Curated capability lane id from `blazar engine offers` —
    ...
    /// `blazar engine offers --arch qwen35` filters to one
    /// architecture. Install one with
    /// `blazar engine install --lane <id>`.
    Offers {
...
enum ConfigCmd {
    ...
    Set {
        ...
        /// a quoted segment stays one segment). `blazar config defaults`
        /// lists every knob; `blazar config edit <model>` hints that
    ...
    /// until restarted; `BLAZAR_*` env overrides still win over the
    /// file either way.
    Unset {
        /// Top-level knob name or dotted table path (see
        /// `config set --help` for the key grammar)
    ...
    /// itself on save.
    Edit {
        /// Model name (from `blazar list`); its engine's knobs become
        /// editor hints
        model: Option<String>,
    },
}
...
Examples:
  blazar config set slots 1        pin a knob (validated before the file is touched)
  blazar config get slots          show the current effective value
  blazar config unset slots        remove the pin, return to the built-in default
  blazar config defaults slots     show the built-in default (omit KEY for all)
  blazar config list               full effective config as TOML
  blazar config set sglang.grammar_backend xgrammar    dotted keys reach table knobs
  blazar config unset sglang.grammar_backend           ...and unset them the same way
  blazar config edit                 open the file in $VISUAL/$EDITOR
  blazar config edit qwen2.5-0.5b    ...with that model's engine knobs as editor hints";

/// `blazar config set --help` footer: what `set` itself accepts — the
...
Examples:
  blazar config set slots 1                      top-level knob
  blazar config set sse_ping_interval -1         ints, bools, floats, \"strings\", [\"lists\"]
  blazar config set sglang.grammar_backend xgrammar    dotted path into a table knob
  blazar config set model_overrides.\"qwen2.5-0.5b\".mistralrs.prefix_cache_n 0    per-model override
  blazar config defaults                         every knob + its built-in default
...
fn render_grouped_help() -> String {
    ...
    writeln!(
        out,
        "blazar {} — multi-engine local inference: llama.cpp, mistral.rs and SGLang orchestrated behind one OpenAI + Ollama + Anthropic gateway",
        env!("CARGO_PKG_VERSION")
    )
    .unwrap();
    writeln!(out).unwrap();
    writeln!(out, "Usage: blazar <COMMAND>").unwrap();
    ...
    writeln!(
        out,
        "\nQuickstart: blazar pull <model> · blazar run <model> · blazar doctor"
    )
    .unwrap();
    writeln!(
        out,
...
fn banner() {
    println!(
        "blazar {} — powered by upstream llama.cpp, mistral.rs and SGLang",
        env!("CARGO_PKG_VERSION")
    );
}
...
/// Exact-basename fallback for `blazar run <file>.gguf`: the user
...
fn match_model_basename(input: &str, rows: &[blazar_core::store::ModelRow]) -> Option<String> {
    if !input.contains('.') {
        return None;
    }
    let hits: Vec<&blazar_core::store::ModelRow> = rows
        .iter()
        .filter(|r| {
...
/// `blazar run` auto-fetch: the store first (with the shared colon
...
fn no_such_model(name: &str) -> anyhow::Error {
    if name.contains(':') {
        anyhow!(
            "no such model: {name} — blazar names are flat; `{}` would be its flat form \
             (see `blazar list`)",
            name.replace(':', "-")
        )
    } else {
        anyhow!("no such model: {name} (see `blazar list`)")
    }
}
...
/// resolver behind `blazar list`'s ENGINE column and `blazar doctor`'s
...
fn engine_install_command(kind: blazar_core::engine_kind::EngineKind) -> String {
    use blazar_core::engine_kind::EngineKind;
    match kind {
        EngineKind::LlamaCpp => "blazar engine update".to_string(),
        other => format!("blazar engine install --kind {other}"),
    }
}
...
fn engine_offer_line(kind: blazar_core::engine_kind::EngineKind) -> String {
    use blazar_core::engine_kind::EngineKind;
    match kind {
        EngineKind::Sglang => {
...
enum LaneState {
    ...
    Missing(Vec<blazar_core::engine_kind::EngineKind>, String),
}

fn lane_state_for(d: &BlazarDirs, row: &blazar_core::store::ModelRow) -> LaneState {
    use blazar_core::engine_kind::{EngineKind, LaneError};
    ...
    match engine_rows.iter().find(|r| r.active) {
        None => LaneState::Missing(
            ...
            "no engine installed — blazar engine update".to_string(),
        ),
        Some(active) => {
...
/// Piped/CI stdin and `BLAZAR_NO_PROMPT=1` print the teaching line
...
async fn offer_missing_engine(d: &BlazarDirs, row: &blazar_core::store::ModelRow) {
    ...
    if !std::io::stdin().is_terminal() || std::env::var_os("BLAZAR_NO_PROMPT").is_some() {
        eprintln!("{teach}");
        return;
    }
...
async fn install_offer_kinds(
    ...
    kinds: &[blazar_core::engine_kind::EngineKind],
) {
    ...
    match config() {
        Ok(cfg) => {
            let legacy = std::fs::read_to_string(d.config_file())
                .is_ok_and(|raw| blazar_core::Config::raw_has_legacy_keys(&raw));
            if legacy {
                checks.push(Check::warn(
                    "config",
                    "parsed with an IN-MEMORY migration (legacy api_keys) — persist with `blazar migrate`".to_string(),
                ));
            } else {
...
fn doctor_routing(d: &blazar_core::dirs::BlazarDirs) -> Vec<Check> {
    ...
    if unservable == 0 {
        checks.push(Check::ok(
            "routing",
            format!(
                "{} mode — all {} pulled model(s) have a serving lane (see `blazar list`)",
                match cfg.engine_routing.mode {
                    blazar_core::config::RoutingMode::Auto => "auto",
                    blazar_core::config::RoutingMode::Manual => "manual",
                },
                models.len()
            ),
        ));
...
fn doctor_engines(d: &BlazarDirs) -> Vec<Check> {
    let mut out = Vec::new();
    let Ok(store) = Store::open(d) else {
        return out;
    };
    ...
    for (kind, label) in [
        ...
        if rows.is_empty() {
            out.push(Check::warn(
                label,
                format!("none installed — `blazar engine install --kind {kind}`"),
            ));
            continue;
        }
    ...
    if over.is_empty() {
        out.push(Check::ok(
            "engine retention",
            format!("every kind ≤ {keep} engine dir(s) — `blazar engine prune` enforces"),
        ));
    } else {
        out.push(Check::warn(
            "engine retention",
            format!("{} — `blazar engine prune` reclaims disk", over.join(", ")),
        ));
    }
    out
}
...
fn doctor_chunking(cfg: &blazar_core::Config) -> Vec<Check> {
...
fn doctor_runtime(d: &BlazarDirs) -> Vec<Check> {
    ...
    if let Ok(o) = show {
        if o.status.success() {
            ...
            if restarts == 0 {
                ...
                out.push(Check::warn(
                    "daemon uptime",
                    format!("up since {since}, {restarts} restart(s) — `journalctl -u blazar -e` for the cause"),
                ));
            }
        } else {
            out.push(Check::warn(
        ...
        .map(|pat| {
            ...
            if let Ok(rd) = std::fs::read_dir("/tmp") {
                for e in rd.flatten() {
                    ...
                    let hit = match *pat {
                        "blazar-census-*" => name.starts_with("blazar-census-"),
                        "blazar-engine-test-*" => name.starts_with("blazar-engine-test-"),
                        _ => name.starts_with("blazar-res-"),
                    };
                    if hit {
                        n += 1;
                    }
    ...
    if stale == 0 {
        out.push(Check::ok(
            "tempdir hygiene",
            "no stale blazar dirs under /tmp".to_string(),
        ));
    } else {
        out.push(Check::warn(
            "tempdir hygiene",
            format!("{stale} stale blazar dir(s) under /tmp — safe to rm"),
        ));
    }
...
fn doctor_model_types(d: &BlazarDirs) -> Vec<Check> {
    ...
    if missing > 0 {
        let _ = write!(
            detail,
            ", {missing} MISSING (stale rows — `blazar rm` or re-pull)"
        );
    }
    if missing > 0 {
...
fn doctor_next_steps(checks: &[Check]) -> Vec<String> {
    ...
    if !port_up {
        steps.push("start the daemon: blazar serve (or: systemctl start blazar)".to_string());
    }
    if no_models {
        steps.push("pull a model: blazar pull <name> (find one: blazar search qwen3)".to_string());
    }
    if steps.is_empty() {
        steps.push("chat: blazar run <model>".to_string());
    }
    steps
}
...
fn doctor_config_pins(cfg: &blazar_core::Config) -> Vec<Check> {
...
fn doctor_keys(d: &BlazarDirs) -> Vec<Check> {
    let mut out = Vec::new();
    let Ok(cfg) = blazar_core::Config::from_toml(
    ...
    if cfg.keys.is_empty() {
        out.push(Check::ok(
            "keys",
            "none configured (authless loopback) — add via `blazar keys add` before exposing the port",
        ));
        return out;
    }
...
fn engine_smoke_check(server_path: &str) -> Check {
        ...
        .output()
    {
        ...
        Ok(out) => Check::warn(
            "engine binary",
            format!(
                "exited {status} — reinstall: blazar engine update",
                status = out.status
            ),
        ),
        Err(e) => Check::warn(
...
fn doctor_exposure(d: &BlazarDirs) -> Vec<Check> {
...
/// rows also note blazar speaks plain HTTP — TLS belongs in a reverse
/// proxy in front.
        ...
        Check::warn(
            ...
            format!(
                "{binding}:{port} reachable off-box with NO auth — anyone on the network can \
                 use this box; set host=127.0.0.1 or add keys (`blazar keys add`); blazar \
...
/// Store health: sqlite `quick_check` on blazar.db. Absent = fresh
/// install (ok row); unreadable or failing = warn with the regenerate
/// hint — usage history is the only casualty.
fn doctor_store(d: &BlazarDirs) -> Vec<Check> {
    let db = d.db_file();
    if !db.is_file() {
        return vec![Check::ok(
            "store",
            "no blazar.db yet (fresh install)".to_string(),
        )];
    }
    ...
    match Store::open(d) {
        Ok(store) => {
            ...
            if verdict == "ok" {
                vec![Check::ok(
                    "store",
                    format!("blazar.db ok ({size_kib} KiB, quick_check passed)"),
                )]
            } else {
                vec![Check::warn(
                    "store",
        ...
        Err(e) => vec![Check::warn(
            "store",
            format!(
                "blazar.db unreadable ({e:#}) — back up, then delete {} to regenerate (usage history lost)",
                db.display()
            ),
        )],
    }
...
/// create. `BLAZAR_SYSTEMCTL` overrides the systemctl binary (test
...
fn doctor_service() -> Vec<Check> {
    let systemctl = std::env::var("BLAZAR_SYSTEMCTL").unwrap_or_else(|_| "systemctl".into());
        ...
        .is_ok()
    {
        let enabled = std::process::Command::new(&systemctl)
            .args(["is-enabled", "blazar"])
            .output()
            .ok()
            .filter(|o| o.status.success())
            .map(|o| String::from_utf8_lossy(&o.stdout).trim() == "enabled");
        let active = std::process::Command::new(&systemctl)
            .args(["is-active", "blazar"])
            .output()
            .ok()
            .filter(|o| o.status.success())
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let label = if uid == "0" {
            "system/dev.blazar".to_string()
        } else {
            format!("gui/{uid}/dev.blazar")
        };
...
fn service_verdict(enabled: Option<bool>, active: Option<bool>, manager: &str) -> Option<Check> {
    match (enabled, active) {
        ...
        (Some(true), Some(false)) => Some(Check::warn(
            "service",
            format!("{manager} unit enabled but not running — logs: journalctl -u blazar"),
        )),
        (Some(false), Some(true)) => Some(Check::warn(
            "service",
            "daemon running but unit not enabled — may not survive reboot; run: sh \
             scripts/install.sh (or systemctl enable blazar)",
        )),
        (Some(false), Some(false)) => Some(Check::ok(
            "service",
            format!("no {manager} unit (manual/dev lane — start with `blazar serve`)"),
        )),
        _ => None,
    }
}
...
#[allow(clippy::too_many_lines)]
async fn doctor_engine(d: &BlazarDirs) -> Vec<Check> {
    let mut checks = Vec::new();
    ...
    if std::env::consts::OS == "linux" {
        let pci = blazar_runtime::probe::pci_gpu_vendors();
    ...
    {
        let detail = match blazar_runtime::engine::gh::engine_overlay_repo() {
            Some(repo) => format!(
                "NVIDIA GPU on the Vulkan asset — prebuilt CUDA engines are preferred \
                 automatically from the overlay at {repo}; set BLAZAR_ENGINE_REPO to \
                 change it"
            ),
            None => "NVIDIA GPU on the Vulkan asset — prebuilt CUDA engines are preferred \
                 ...
                 assets); run: blazar engine update"
                .to_string(),
        };
...
fn active_engine_asset(d: &BlazarDirs, active_tag: Option<&str>) -> String {
...
fn active_asset_is_cuda(d: &BlazarDirs, active_tag: Option<&str>) -> bool {
...
fn marker_channel_matches(
    ...
    cfg: blazar_core::config::UpdateChannel,
) -> bool {
...
use blazar_runtime::engine::gh::vtag_semver;
...
fn overlay_lag_note(active_tag: &str, asset: &str) -> &'static str {
    if active_tag.ends_with("-cuda") && !asset.starts_with("built-") {
        " (CUDA prebuilts may lag the newest build while the overlay repo \
         catches up; when `blazar engine update` reports the prebuilt lane \
         is behind, `blazar engine build cuda` compiles the newest build \
         locally)"
    } else {
        ""
    }
        ...
        Ok(Ok(rel)) => {
            if rel.tag_name == active {
                ...
                Check::warn(
                    "engine currency",
                    format!(
                        "{} available: {} (active: {}, mistral.rs) — run: blazar engine update --kind mistralrs",
                        channel_word(active, &rel.tag_name),
                        rel.tag_name,
                        active
                    ),
...
fn cuda_opportunity_rows(
        ...
        rows.push(Check::ok(
            ...
            format!(
                "NVIDIA GPU (driver CUDA {dmaj}.{dmin}{cc}) — a CUDA engine is faster: \
                 `blazar engine build cuda` (compiles upstream) or \
                 `blazar engine install` (mistral.rs prebuilt)"
            ),
        ));
    }
    ...
    if missing.is_empty() {
        rows.push(Check::ok(
            "cuda toolchain",
            "ready (git, cmake, nvcc) — `blazar engine build cuda` can run now",
        ));
    } else {
...
fn gpu_driver_rows(
    pci_vendors: &[String],
    system_vendor: blazar_runtime::engine::manifest::Vendor,
    ...
    if pci_vendors.iter().any(|v| v == "10de") && system_vendor != Vendor::Nvidia {
        rows.push(Check::warn(
            ...
            "NVIDIA GPU present (PCI 10de:) but no NVIDIA driver userspace (nvidia-smi missing) \
             ...
             installs the distro driver from first-party repos), reboot, then `blazar engine \
...
/// App currency: the newest blazar release on the same repo/URL the
/// `blazar upgrade` lane installs from. Warn-only — never installs
/// (`blazar upgrade` stays a human action, exactly like engine updates).
/// `BLAZAR_REPO` unset → informational row (self-upgrade lane not
/// configured; dev checkouts and manual installs live here).
async fn doctor_app_currency() -> Vec<Check> {
    let Some(repo) = std::env::var("BLAZAR_REPO").ok().filter(|r| !r.is_empty()) else {
        return vec![Check::ok(
            "app currency",
            "BLAZAR_REPO unset — self-upgrade lane not configured",
        )];
    };
    let base = std::env::var("BLAZAR_INSTALL_BASE_URL")
        .unwrap_or_else(|_| "https://api.github.com".to_string());
    let token = std::env::var("GH_TOKEN")
        .or_else(|_| std::env::var("GITHUB_TOKEN"))
        .ok();
    let Ok(gh) = blazar_runtime::GhClient::with_base(&base, token) else {
...
async fn doctor_whisper_currency(d: &BlazarDirs) -> Vec<Check> {
    ...
    let token = std::env::var("GH_TOKEN")
        .or_else(|_| std::env::var("GITHUB_TOKEN"))
        .ok();
    let Ok(gh) = blazar_runtime::GhClient::new(token) else {
        return vec![Check::warn(
            "whisper currency",
                ...
                Check::warn(
                    ...
                    format!(
                        "update available: {latest} (pinned: {pin}) — run: \
                         blazar whisper --install --tag {latest}"
                    ),
                )
            }
        }
...
fn doctor_whisper_models(d: &BlazarDirs) -> Vec<Check> {
    if blazar_runtime::whisper::server_bin(d).is_none() {
        return Vec::new(); // covered by the "whisper lane" row above
    }
    let pulled = blazar_runtime::whisper::list_models(d);
    if pulled.is_empty() {
        vec![Check::warn(
            "whisper models",
            "none pulled — transcription will 501 until `blazar whisper --pull base`",
        )]
    } else {
        vec![Check::ok(
            "whisper models",
...
/// Multiple blazar binaries on one box: your shell resolves one order,
...
fn doctor_binary_shadow() -> Vec<Check> {
    ...
    if found.len() <= 1 {
        return vec![Check::ok("binary", "single blazar on PATH".to_string())];
    }
    ...
    if stale {
        vec![Check::warn(
            "binary",
            format!(
                "multiple blazar binaries; system services with their own PATH resurrect the stale copy:{detail}\n  fix: sudo rm the stale path (or reinstall with --system)"
            ),
        )]
    } else {
        vec![Check::warn(
            "binary",
            format!("multiple blazar binaries on PATH:{detail}"),
        )]
    }
}
...
#[cfg(unix)]
fn doctor_disk(d: &BlazarDirs) -> Vec<Check> {
...
fn doctor_sentinel(d: &BlazarDirs) -> Vec<Check> {
    ...
    // `blazar why` of record used to be a dead end whenever the flagged
    // population sat older than the newest 10 clean records.
    let top_code = counts
        .iter()
        .max_by_key(|(_, n)| *n)
        .map_or_else(String::new, |(c, _)| format!(" (`blazar why --code {c}`)"));
    vec![Check::warn(
        "sentinel",
        format!(
            "{flagged} of {records} requests in 24h flagged: {} — `blazar why --flagged` for details + retry hints{top_code}",
            top.join(", ")
        ),
    )]
}
...
fn orphan_scan(models: &[blazar_core::store::ModelRow], dir: &Path) -> OrphanReport {
...
fn doctor_models(d: &BlazarDirs) -> Vec<Check> {
    ...
    let orphan_suffix = |r: &OrphanReport| -> String {
        ...
        if !r.orphans.is_empty() {
            ...
            write!(
                s,
                "; {} unmanaged file(s): {}{} — `blazar import <file> --name <n>` to register, or delete to reclaim",
                r.orphans.len(),
                shown.join(", "),
                extra
            )
...
/// (`BLAZAR_VALIDATE=1`): the runtime installs a Linux PDEATHSIG
/// parent-death guard; see `blazar_runtime::validate_parent_death_guard`.
async fn serve() -> Result<()> {
    ...
    let _lock = match blazar_runtime::DaemonLock::acquire(&d) {
        Ok(lock) => lock,
    ...
    let reconcile = blazar_runtime::models::reconcile_models(&d, &store);
    ...
    // Boot preflight (2/2): VRAM already held before blazar spawns any
    // child of its own. Advisory only — blazar never kills another
    // product's process.
    ...
    let engine: Arc<dyn Engine> = match engine_row.kind {
        blazar_core::engine_kind::EngineKind::MistralRs => Arc::new(MistralRsEngine::with_staging(
            manifest,
            engine_env,
    ...
    let state = Arc::new(blazar_gateway::state::AppState::new(
        d.clone(),
        cfg.clone(),
        sup.clone(),
        bus,
    ...
    // stale-copy race (gateways auto-start blazar from their own PATH).
    ...
    // stay up; budgets/scopes apply on the next request).
    {
        ...
        let hook: std::sync::Arc<dyn Fn() + Send + Sync> = std::sync::Arc::new(move || {
            ...
            if let Ok(fresh) = blazar_core::Config::from_toml(&raw) {
                for k in &fresh.keys {
        ...
        *blazar_runtime::daemon::SIGHUP_HOOK.lock().expect("hook") = Some(hook);
    }
    ...
    let served = blazar_gateway::serve(
        state.clone(),
        &host,
        port,
        Box::pin(blazar_runtime::wait_for_shutdown_signal()),
    )
    .await;
    flusher.abort();
    state.keys.flush(&d);
...
fn stop() -> Result<()> {
    let d = dirs();
    let pidfile = d.run_dir().join("blazar.pid");
    ...
    // a grace beat. Never a group — exact pid only, like the unix lane.
    {
        ...
        let still_alive = blazar_runtime::process_alive_by_pid(u32::try_from(pid).unwrap_or(0));
        if still_alive {
...
async fn pull(target: &str, force: bool, verify: bool) -> Result<()> {
    ...
    if verify {
        return match blazar_runtime::verify_model(&dirs(), target).await? {
...
/// The pull flow without the banner or the summary line, so `blazar
...
async fn pull_model(target: &str, force: bool) -> Result<(blazar_core::store::ModelRow, bool)> {
    let d = dirs();
    d.ensure().ok();
    ...
    let outcome = tokio::select! {
        ...
        () = blazar_runtime::events::interrupted() => {
            return Err(anyhow::anyhow!(
                "pull interrupted — partial file kept; re-run `blazar pull {target}` to resume"
            ));
        }
    };
    let row = outcome.row;
    ...
    while let Ok(ev) = events.try_recv() {
        if let blazar_runtime::BlazarEvent::ModelPulled {
            warning: Some(w), ..
        } = ev
        {
            println!("WARNING: {w}");
...
fn mmproj_cmd(model: &str, path: &std::path::Path) -> Result<()> {
    let d = dirs();
    d.ensure().ok();
    if blazar_runtime::models::instance_running(&d, model) {
        return Err(anyhow!(
            "model {model} is currently running; stop it first (`blazar stop {model}` or wait for eviction)"
        ));
    }
    let store = Store::open(&d)?;
    let mut row = store
    ...
    let mmproj_meta = blazar_core::read_metadata_file(path)
...
fn import(
    ...
    let d = dirs();
    d.ensure().ok();
    let meta = blazar_core::read_metadata_file(path.as_path())
    ...
    blazar_runtime::models::ensure_portable_name(&model_name)?;
    ...
    let mmproj_dest = mmproj
        .as_ref()
        .map(|p| {
            let proj_meta = blazar_core::read_metadata_file(p.as_path())
...
/// (unexpected — `blazar show <name>` to inspect); `missing!` = the
...
fn show(model: &str, json: bool) -> Result<()> {
    ...
    let row = store
        .get_model(model)?
        .ok_or_else(|| no_such_model(model))?;
    let meta = blazar_core::read_metadata_file(std::path::Path::new(&row.path)).ok();
...
fn ps_row_cells(m: &serde_json::Value) -> (String, String, String) {
    ...
    let gpu = match m["blazar_device"].as_str() {
        Some(dev) => format!("{}@{}", m["blazar_gpu"].as_str().unwrap_or("-"), dev),
        None => m["blazar_gpu"].as_str().unwrap_or("-").to_string(),
    };
    ...
    let spec = match (
        m["blazar_spec"].as_str().unwrap_or(""),
        m["blazar_draft"].as_str(),
    ) {
        ("", _) => "-".to_string(),
        (mode, Some(draft)) => format!("{mode}+{draft}"),
        (mode, None) => mode.to_string(),
    };
    let hit = match m["blazar_cache_hit"].as_f64() {
...
async fn ps(reset: bool, json: bool) -> Result<()> {
    ...
    if json {
        ...
        for m in models {
            println!(
                ...
                serde_json::json!({
                    "name": m["name"].as_str(),
                    "replica": m["blazar_replica"].as_i64(),
                    ...
                    "in_flight": m["blazar_in_flight"].as_i64().unwrap_or(0),
                    "endpoint": m["blazar_endpoint"].as_str().unwrap_or("-"),
                    "warnings": m["blazar_warnings"].as_array().cloned()
                        .unwrap_or_default(),
                })
            );
        }
    ...
    if let (Ok(pid), Ok(path)) = (
        std::fs::read_to_string(d.run_dir().join("blazar.pid")),
    ...
    for m in models {
        ...
        println!(
            ...
            spec,
            hit,
            m["blazar_in_flight"].as_i64().unwrap_or(0),
            m["blazar_endpoint"].as_str().unwrap_or("-")
        );
        ...
        if let Some(ws) = m["blazar_warnings"].as_array() {
            for w in ws {
...
/// `blazar why [trace]` — sentinel ring dump: what the model returned,
...
/// `blazar watch` — live SSE tail of sentinel records. One line per
...
async fn watch() -> Result<()> {
    ...
    let mut chunk_lines = blazar_gateway::translate::LineBuffer::new();
...
/// `blazar cp SOURCE DEST` — zero-byte hardlink alias.
...
/// `blazar rm A B C` — continue past failures, report all, exit non-zero.
fn rm_multi(models: &[String]) -> Result<()> {
    let d = dirs();
    let mut failed = Vec::new();
    for m in models {
        if let Err(e) = blazar_runtime::models::remove_model(&d, m) {
            eprintln!("rm {m}: {e:#}");
            failed.push(m.clone());
        } else {
            println!("removed {m}");
...
/// Modelfile subset blazar understands. Everything else is collected and
...
/// `blazar create MODEL -f Modelfile` — the anti-ollama version: no blob
...
fn create_cmd(model: &str, file: Option<&std::path::Path>) -> Result<()> {
    ...
    if !spec.unsupported.is_empty() {
        return Err(anyhow!(
            "Modelfile keys blazar refuses to fake: {}\n\
blazar has no Modelfile layer: GGUF is truth (templates embedded, served via --jinja) and \
parameters are per-request or per-model config overlays. Supported subset: FROM, PARAMETER num_ctx, ADAPTER. \
Alternatives: model_overrides in ~/.config/blazar/config.toml, or per-request options.",
            spec.unsupported.join(", ")
        ));
    }
    let d = dirs();
    blazar_runtime::models::copy_model(&d, &spec.base, model)?;
    // Overlay: ctx + loras for the new alias.
    let cfg_path = d.config_file();
    let cfg = if cfg_path.exists() {
        let raw = std::fs::read_to_string(&cfg_path)?;
        blazar_core::Config::from_toml(&raw).map_err(|e| anyhow!("{e}"))?
    } else {
    ...
    blazar_core::persist_config(&cfg_path, &cfg.to_toml().map_err(|e| anyhow!("{e}"))?)?;
    println!(
...
/// `blazar keys [list]|add|rm` — HTTP client to the daemon's
...
/// first non-empty `name:key` from `BLAZAR_KEYS`, else the first unscoped
/// (admin) [[keys]] entry from config.toml.
fn admin_bearer() -> Option<String> {
    if let Ok(v) = std::env::var("BLAZAR_KEYS") {
        ...
        for entry in v.split(',').map(str::trim).filter(|s| !s.is_empty()) {
            match entry.split_once(':') {
                ...
                None => eprintln!(
                    "blazar: ignoring malformed BLAZAR_KEYS entry '{entry}' \
...
async fn keys_cmd(action: Option<KeysAction>) -> Result<()> {
    ...
    // BLAZAR_KEYS entries are admin by construction; otherwise the
...
/// `blazar quantize <model> -t Q4_K_M` — run the engine's own
...
fn quantize_cmd(
    ...
    if !blazar_runtime::quantize::plausible_quant_type(qtype) {
        return Err(anyhow!(
    ...
    let row = store
        .get_model(model)?
        .ok_or_else(|| anyhow!("unknown model {model:?} (blazar list)"))?;
    let bin = blazar_runtime::quantize::find_quantize_bin(&d)?;
    ...
    let mut imatrix_args: Vec<String> = match &imatrix {
        Some(calib) => {
            let ibin = blazar_runtime::quantize::find_imatrix_bin(&d)?;
            ...
            let im = blazar_runtime::quantize::imatrix(
                &ibin,
    ...
    let out = blazar_runtime::quantize::quantize_im(
        &bin,
        std::path::Path::new(&row.path),
        &tmp.path,
        qtype,
    ...
    if let Some(max_degradation_pct) = verify_gate_pct {
        let pbin = blazar_runtime::quantize::find_perplexity_bin(&d)?;
        let probe = blazar_runtime::quantize::write_verify_probe(&d)?;
        println!("verify: perplexity pass 1/2 (base) — full forward passes, this takes a while...");
        let base_ppl = blazar_runtime::quantize::perplexity(
            &pbin,
            std::path::Path::new(&row.path),
            &probe,
            |line| {
        ...
        let out_ppl = blazar_runtime::quantize::perplexity(&pbin, &out, &probe, |line| {
            println!("  {line}");
        })?;
        if let Err(e) =
            blazar_runtime::quantize::verify_gate(base_ppl, out_ppl, max_degradation_pct / 100.0)
        {
    ...
    let meta = blazar_core::read_metadata_file(&out)
...
/// `blazar launch <cli> [args...]` — ollama-`launch`-class ergonomics
...
async fn launch_cmd(command: Vec<String>, warm: Option<String>, key: Option<String>) -> Result<()> {
    let Some(program) = command.first().cloned() else {
        return Err(anyhow!(
            "usage: blazar launch [--warm MODEL] [--key KEY] <cli> [args...]\n\
             examples: blazar launch claude\n\
                        blazar launch --warm qwen3.5-9b dsh\n\
...
/// `blazar drafts <model>` — speculative-decoding draft candidates:
...
async fn drafts_cmd(model: &str) -> Result<()> {
    let client = blazar_runtime::hf::HfClient::new(std::env::var("HF_TOKEN").ok())?;
...
/// `blazar migrate` — one-click config persistence: parses (which
...
fn migrate_cmd() -> Result<()> {
    ...
    blazar_core::persist_config(&path, &cfg.to_toml()?)?;
    println!(
...
/// `blazar snapshot` — backup the small state (config, store,
/// sessions list) to `<data>/snapshots/<ts>/`. Models/engines stay in
/// place (they ARE the bulk; re-pull or re-copy them deliberately).
/// Snapshots kept on disk; older ones are pruned on each `blazar snapshot`.
...
fn snapshot_cmd() -> Result<()> {
    ...
    if d.db_file().exists() {
        ...
        let store = Store::open(&d)?;
        store
            .snapshot_db_to(&dir.join("blazar.db"))
            .with_context(|| "snapshot blazar.db (VACUUM INTO)".to_string())?;
        copied.push("blazar.db".to_string());
    }
...
fn prune_snapshots(d: &BlazarDirs, keep: usize) {
...
/// `blazar coreside` — which local models can stay loaded together.
fn coreside_cmd() -> Result<()> {
    ...
    let models = store.list_models()?;
    if models.is_empty() {
        return Err(anyhow!("no models pulled (blazar pull <model>)"));
    }
    let manifest = store
        .active_engine()
        .ok()
        .flatten()
        .and_then(|e| serde_json::from_str::<blazar_runtime::Manifest>(&e.manifest).ok());
    let vram = manifest.as_ref().map_or(0, |m| {
        blazar_runtime::probe_hardware(Some(m)).total_vram_mib()
    });
    if vram == 0 {
        return Err(anyhow!(
    ...
    for m in &models {
        ...
        let kv = blazar_core::read_metadata_file(std::path::Path::new(&m.path)).map_or(0, |meta| {
            blazar_core::coreside::kv_f16_mib(&meta, u64::from(ctx))
        });
        fps.push(blazar_core::coreside::Footprint {
            name: m.name.clone(),
...
async fn tts_cmd(
    ...
    if let Some(value) = pin {
        ...
        blazar_runtime::piper::set_pin(&d, target)?;
        if unpin {
    ...
    if list {
        match blazar_runtime::piper::server_bin(&d) {
            Some((bin, _)) => {
                let pin = if blazar_runtime::piper::pinned_tag(&d).is_some() {
                    " (pinned)"
                } else {
                    ""
                };
            ...
            None => println!("server: not installed (blazar tts --install)"),
        }
        let voices = blazar_runtime::piper::list_voices(&d);
        if voices.is_empty() {
            println!("voices: none pulled (blazar tts --pull en_US-amy-medium)");
        } else {
            for v in voices {
                println!("voice: {v}");
            }
...
async fn tts_speak(
    ...
    let voice = if let Some(v) = voice {
        ...
        blazar_runtime::piper::list_voices(d)
            .first()
            .cloned()
...
async fn whisper_cmd(
    ...
    if let Some(value) = pin {
        let unpin = value.trim().eq_ignore_ascii_case("none");
        let target = if unpin { None } else { Some(value.as_str()) };
        blazar_runtime::whisper::set_pin(&d, target)?;
        if unpin {
    ...
    if list {
        ...
        let models = blazar_runtime::whisper::list_models(&d);
        if models.is_empty() {
            println!("models: none (blazar whisper --pull base)");
        } else {
            println!("models:");
            for m in models {
                println!("  {m}");
    ...
    let local_ready = blazar_runtime::whisper::server_bin(&d).is_some()
        && !blazar_runtime::whisper::list_models(&d).is_empty();
...
async fn stream_chat(base: String, body: serde_json::Value) -> Result<StreamOutcome> {
    ...
    if rendered_any {
        ...
        eprintln!(
            "blazar: {model} returned an empty response — no content arrived \
             (try a different quant or engine lane: blazar engine list)"
        );
    }
    Ok(StreamOutcome {
        interrupted,
...
fn tune_full(
    ...
    if search {
        ...
        let winning = blazar_core::TuningOverrides {
            ctx: Some(n),
            ..Default::default()
        };
        let fixed = tuner.adopt(&store2, &input, &winning)?;
    ...
    if let Some(clients) = slots {
        ...
        let profile = store2
            ...
            .ok_or_else(|| anyhow!("no profile yet — run `blazar tune {model} --search` first"))?;
    ...
    if ngram {
        ...
        let input2 = blazar_runtime::bench::build_input(
            model,
            &row.path,
            u64::try_from(row.bytes.max(0)).unwrap_or(u64::MAX),
            blazar_core::ModelMeta::Gguf(&gguf),
    ...
    if load || replicas || cache_reuse {
        ...
        let draft3 = blazar_runtime::resolve_draft_path(&store, model, &spec_mode3);
        let input3 = blazar_runtime::bench::build_input(
            model,
            &row.path,
            u64::try_from(row.bytes.max(0)).unwrap_or(u64::MAX),
            blazar_core::ModelMeta::Gguf(&gguf),
        ...
        if cache_reuse {
            ...
            // long repeated prompt across {0, 256, 512}. 256 is Blazar's
...
fn resolve_model_and_engine(
    model: &str,
) -> Result<(String, blazar_core::engine_kind::EngineKind, String)> {
    use blazar_core::engine_kind::EngineKind;
    let d = dirs();
    if !d.db_file().is_file() {
        return Err(anyhow!(
            "no blazar store yet — start the daemon once before asking for {model}'s knobs"
        ));
    }
    let store = blazar_core::store::Store::open(&d).map_err(|e| anyhow!("{e}"))?;
...
/// Editor hint block for `blazar config edit <model>`: a comment-only
...
fn knob_hint_block(model: &str, kind: blazar_core::engine_kind::EngineKind, tag: &str) -> String {
    ...
    let mut out: Vec<String> = vec![format!(
        "# --- blazar knob hints for {model} (safe to delete) ---"
    )];
    let lane = match kind {
    ...
    out.push("# full knob surface: `blazar config defaults`".into());
...
fn strip_hint_block(raw: &str, model: &str) -> String {
    let start = format!("# --- blazar knob hints for {model} (safe to delete) ---");
        ...
        if new_tg < prev_tg * 0.9 {
            ...
            anyhow::bail!(
                "REGRESSION GATE TRIPPED: {:.0}% decode drop on {model} — \
                 ...
                 installed for `blazar engine use {row_tag}` to force.",
    ...
    match cmd {
        ...
        EngineCmd::List { json } => {
            ...
            for e in store.list_engines()? {
                ...
                let m = serde_json::from_str::<blazar_runtime::Manifest>(&e.manifest).ok();
                let prov = m
                    .as_ref()
        ...
        EngineCmd::Use { tag, kind } => {
            let resolved_tag = match (tag, kind) {
                ...
                (None, Some(kind)) => {
                    use std::str::FromStr;
                    let wanted = blazar_core::engine_kind::EngineKind::from_str(&kind)
                        .map_err(|e| anyhow!("--kind {kind}: {e}"))?;
                    let rows = blazar_core::store::Store::open(&d)?.list_engines()?;
                    ...
                    rows.iter()
                        ...
                        .ok_or_else(|| {
                            anyhow!(
                                "no {kind} engine installed — blazar engine install --kind {kind}"
                            )
                        })?
                }
                (Some(_), Some(_)) => {
...
/// `blazar engine install` — install + activate a prebuilt mistral.rs
...
/// `blazar engine update --kind mistralrs [tag]` — lane parity with the
...
async fn engine_update_mistralrs(d: &BlazarDirs, tag: Option<String>, check: bool) -> Result<()> {
    let store = Store::open(d)?;
    let Some(installed) = newest_mistralrs_tag(&store)? else {
        return Err(anyhow!(
            "no mistralrs engine installed — `blazar engine install --kind mistralrs` first"
        ));
    };
    let pinned = tag.is_some();
...
/// `blazar engine update --kind sglang [version]` — pip venv lane
...
async fn engine_update_sglang(d: &BlazarDirs, version: Option<String>, check: bool) -> Result<()> {
    ...
    let installed = store
        ...
        .max_by_key(|e| {
            blazar_runtime::engine::sglang_install::version_tuple(
...
/// `blazar engine update` — install + activate the newest (or given)
...
async fn engine_update(
                ...
                Some(overlay) => match lane.newest_cuda {
                    Some((need_maj, need_min)) => println!(
                        "overlay {overlay} published, but its CUDA {need_maj}.{need_min} assets \
                         exceed this driver ({maj}.{min}) — `blazar engine build cuda` compiles \
                         locally for this driver"
                    ),
                    None => println!(
                        "no CUDA overlay release {overlay} published yet — rerun after \
                         the overlay publishes, or `blazar engine build cuda`"
                    ),
                },
    ...
    // Skip: --no-gate, BLAZAR_ENGINE_GATE=0, or channel
    // downgrade (an older build losing to a newer one is the
    // point of the switch, not a regression).
    let gate_on =
        !no_gate && std::env::var("BLAZAR_ENGINE_GATE").as_deref() != Ok("0") && !downgrade;
    if unchanged {
    ...
    if row.active {
        ...
        println!(
            "engine {} registered (build {}, {} devices, {} flags); the active CUDA \
             engine was kept — run `blazar engine use {}` to switch",
            row.tag,
            m.build_number,
            m.devices.len(),
            m.flags.len(),
...
/// `blazar engine build` — compile llama.cpp from source into an
...
/// and runs with the user's privileges and Blazar cannot audit it.
fn fork_lane_source(a: &BackendArg) -> Result<Option<BuildSource>> {
    ...
    println!(
        "TRUST: building code from fork {repo}@{sha} — it compiles and runs on this \
         ...
         (commit {sha}); Blazar records but cannot audit fork code."
    );
...
fn engine_gate_on(
    ...
    if fork || a.no_gate || std::env::var("BLAZAR_ENGINE_GATE").as_deref() == Ok("0") {
...
fn print_fork_lane_howto(row: &blazar_core::EngineRow, m: &blazar_runtime::Manifest) {
    println!(
...
async fn resolve_build_label(
    gh: &GhClient,
    channel: blazar_core::config::UpdateChannel,
    a: &BackendArg,
    source: &BuildSource,
) -> Result<String> {
    match (&a.tag, source) {
...
async fn engine_build(d: &BlazarDirs, a: BackendArg) -> Result<()> {
...
async fn refresh_after_lane_change(mgr: &EngineManager, cfg: &blazar_core::Config) {
...
async fn engine_offers(arch: Option<&str>, json: bool) -> Result<()> {
    ...
    println!("install one: blazar engine install --lane <ID> --backend cpu|cuda");
    Ok(())
}

/// `blazar engine install --lane <id>` — build a curated capability
...
async fn engine_install_lane(d: &BlazarDirs, lane_id: &str, backend: Option<&str>) -> Result<()> {
    use blazar_runtime::engine::capability_registry as reg;
    let cfg = config()?;
    ...
    if lane.status == reg::LaneStatus::Retired {
        return Err(anyhow!(
            "registry lane {lane_id} is retired — upstream llama.cpp already ships this \
             capability; `blazar engine update` serves it"
        ));
    }
    ...
    println!(
        "TRUST: building code from fork {}@{} (curated registry lane {}) — it compiles and \
         ...
         not Blazar-audited; the pin is immutable (commit {}).",
    ...
    opts.trust = blazar_runtime::engine::manifest::TrustTier::Curated;
    let row = mgr
        .build_and_install(&opts, &mut |line| println!("  {line}"))
        .await?;
    let m: blazar_runtime::Manifest = serde_json::from_str(&row.manifest)?;
...
/// doctor/ps surfaces read. Never auto-installs — `blazar engine update`
/// stays a human action; failures log at debug and retry next tick.
fn spawn_engine_check_task(
    dirs: &BlazarDirs,
    every_secs: u64,
    engine: Arc<dyn blazar_runtime::Engine>,
) {
    let dirs = dirs.clone();
    tokio::spawn(async move {
        ...
        loop {
            ...
            if active.tag == blazar_runtime::LOCAL_TAG {
                delay = full_delay;
...
fn engine_rm(d: &BlazarDirs, tag: &str) -> Result<()> {
    ...
    let row = store
        ...
        .into_iter()
        .find(|e| e.tag == tag)
        .ok_or_else(|| anyhow!("no such engine: {tag} (see `blazar engine list`)"))?;
    if row.active {
        anyhow::bail!(
            "engine {tag} is active — `blazar engine use <other>` first \
...
fn ps_rows_using_engine(ps: &serde_json::Value, tag: &str) -> Vec<String> {
    ps["models"].as_array().map_or(Vec::new(), |rows| {
        rows.iter()
            .filter(|m| m["blazar_engine"].as_str() == Some(tag))
...
/// `blazar.pid` does not count). Pure over file names so the
...
fn any_child_pidfile(names: impl IntoIterator<Item = String>) -> bool {
    names.into_iter().any(|f| {
        ...
        pid && !f.eq_ignore_ascii_case("blazar.pid")
    })
}
...
/// (`blazar_engine` per row); when the daemon is not answering, any
...
async fn refuse_if_engine_in_use(d: &BlazarDirs, tag: &str) -> Result<()> {
    let cfg = config()?;
    let base = daemon_base(&cfg);
    let http = reqwest::Client::builder()
        .timeout(Duration::from_secs(2))
        ...
        if !users.is_empty() {
            anyhow::bail!(
                "engine {tag} is serving: {} — stop it first \
                 (`blazar stop <model>` or wait for idle eviction)",
                users.join(", ")
            );
        }
        return Ok(());
...
/// for `blazar pull` (`…-heretic-i1-GGUF` vs `…-heretic-GGUF` are
        ...
        if results.is_empty() {
            ...
            if !json {
                let scope = if query.is_empty() {
                    format!(
                        "the popular browse (try a model query: blazar search llama {})",
                        filters[0]
                    )
                } else {
...
fn entry_quants(r: &blazar_runtime::hf::SearchEntry) -> Vec<String> {
    let names = blazar_runtime::hf::quant_tokens(r.siblings.iter().map(|s| s.rfilename.as_str()));
    if names.is_empty() {
        quant_markers(&r.id)
    } else {
        names
...
fn split_quant_query(query: &str) -> (String, Vec<String>) {
    ...
    for token in query.split_whitespace() {
        if blazar_runtime::hf::is_quant_token(token) {
...
fn format_of_entry(r: &blazar_runtime::hf::SearchEntry) -> String {
...
fn format_of_siblings(siblings: &[blazar_runtime::hf::HfSibling]) -> String {
    let ends_with_any =
...
fn entry_size_bytes(r: &blazar_runtime::hf::SearchEntry) -> Option<u64> {
    r.gguf.as_ref().and_then(|g| g.total_file_size).or_else(|| {
        r.safetensors
            .as_ref()
            .and_then(blazar_runtime::hf::HfSafetensorsInfo::byte_estimate)
    })
}

/// ARCH for a search row: gguf architecture > config arch hint
/// (`model_type` > architectures[0] > diffusers pipeline class).
fn entry_arch(r: &blazar_runtime::hf::SearchEntry) -> Option<String> {
    r.gguf
        .as_ref()
        ...
        .or_else(|| {
            r.config
                .as_ref()
                .and_then(blazar_runtime::hf::HfConfigSummary::arch_hint)
        })
}
    ...
    match cmd {
            ...
            match removed {
                ...
                None => Err(anyhow!(
                    "unknown config key: {key} (table knobs use dotted keys: \
                     blazar config set sglang.stream_interval 1)"
                )),
            }
        }
        ...
        ConfigCmd::Edit { model } => {
            ...
            let editor = std::env::var("VISUAL")
                ...
                .map_err(|_| {
                    anyhow!("no editor set: export EDITOR (or VISUAL) to your editor, e.g. EDITOR=nano blazar config edit")
                })?;
            let d = dirs();
            d.ensure().ok();
            let path = d.config_file();
...
/// `blazar upgrade [--version vN] [--dry-run]`: self-update from GitHub
/// Releases with the same digest verification as `blazar engine update`.
async fn upgrade(version: Option<String>, dry_run: bool) -> Result<()> {
    let repo = std::env::var("BLAZAR_REPO")
        .ok()
        .filter(|r| !r.is_empty())
        .ok_or_else(|| {
            anyhow!("BLAZAR_REPO is not set; export BLAZAR_REPO=owner/blazar (the repo hosting blazar releases)")
        })?;
    let base = std::env::var("BLAZAR_INSTALL_BASE_URL")
        .unwrap_or_else(|_| "https://api.github.com".to_string());
    let token = std::env::var("GH_TOKEN")
...
mod tests {
    ...
    fn unit__run_no_draft__parses_before_prompt_only() {
        ...
        let cli = Cli::try_parse_from(["blazar", "run", "m1", "--no-draft", "hi"]).unwrap();
        match cli.cmd {
            Cmd::Run {
                no_draft, prompt, ..
            } => {
        ...
        let cli = Cli::try_parse_from(["blazar", "run", "m1", "hi", "--no-draft"]).unwrap();
        match cli.cmd {
            Cmd::Run {
                no_draft, prompt, ..
            } => {
    ...
    fn unit__format_of_entry__untagged_mlx_detected_from_repo_id() {
        ...
        let entry = |id: &str, tags: &[&str], files: &[&str]| blazar_runtime::hf::SearchEntry {
            id: id.to_string(),
            downloads: None,
            likes: None,
            siblings: files
                .iter()
                .map(|f| blazar_runtime::hf::HfSibling {
    ...
    fn unit__entry_size_and_arch__safetensors_rows_carry_real_metadata() {
        ...
        let hist = |pairs: &[(&str, u64)]| blazar_runtime::hf::HfSafetensorsInfo {
            parameters: Some(
                pairs
                    .iter()
        ...
        let base = |gguf: Option<blazar_runtime::hf::HfGgufInfo>,
                    st: Option<blazar_runtime::hf::HfSafetensorsInfo>,
                    cfg: Option<blazar_runtime::hf::HfConfigSummary>| {
            blazar_runtime::hf::SearchEntry {
                id: "x/y".to_string(),
                downloads: None,
                likes: None,
                siblings: Vec::new(),
        ...
        assert_eq!(
            entry_size_bytes(&base(
                Some(blazar_runtime::hf::HfGgufInfo {
        ...
        let cfg = |mt: Option<&str>, archs: Option<&[&str]>, pipeline: Option<&str>| {
            blazar_runtime::hf::HfConfigSummary {
                architectures: archs
                    .map(|a| a.iter().map(std::string::ToString::to_string).collect()),
                model_type: mt.map(str::to_string),
                diffusers: pipeline.map(|p| blazar_runtime::hf::HfDiffusersSummary {
    ...
    /// blazar-core source (`include_str!`) because serde has no field
    ...
    fn unit__knob_hint_block__covers_every_tuning_field() {
        use blazar_core::engine_kind::EngineKind;
        let core_src = include_str!("../../blazar-core/src/config.rs");
    ...
    fn unit__strip_hint_block__roundtrip_and_mangled() {
        ...
        let block = knob_hint_block("m", blazar_core::engine_kind::EngineKind::Sglang, "t");
    ...
    fn unit__routed_engine_lane__mirrors_serving_lane_contract() {
        ...
        let dir = std::env::temp_dir();
        let auto = blazar_core::Config::from_toml("[engine_routing]\nmode = \"auto\"\n").unwrap();
        assert_eq!(
            routed_engine_lane(
                &auto,
                Some(&global),
    ...
    fn unit__currency_verdict__genuine_pending_update_warns_with_current_active() {
        ...
        assert_eq!(
            c.detail,
            "upgrade available: b10831 (active: b10819, channel: latest) — run: blazar engine update"
        );
    }
    ...
    fn unit__currency_verdict__lane_suffix_same_build_and_lane_aware_hint() {
        ...
        assert!(
            c.detail.contains("run: blazar engine build cuda"),
            "{}",
            c.detail
        );
        ...
        assert!(
            c.detail.contains("run: blazar engine update"),
            "{}",
            c.detail
        );
    ...
    fn unit__currency_verdict__overlay_lane_teaches_lag_and_local_escape() {
        // Live incident: doctor said "b10969 available — run: blazar
        ...
        assert!(
            c.detail.contains("blazar engine build cuda"),
    ...
    fn unit__version_currency_verdict__update_available_warns_with_run_hint() {
        let c = version_currency_verdict("app currency", "0.3.0", "v0.4.0", "blazar upgrade");
        assert!(c.warn);
        assert_eq!(
            c.detail,
            "update available: v0.4.0 (running: 0.3.0) — run: blazar upgrade"
        );
        ...
        assert_eq!(
            w.detail,
            "update available: v1.7.6 (running: v1.7.5) — run: blazar whisper --install"
        );
    }

    #[test]
    fn unit__version_currency_verdict__equal_versions_ok_with_v_prefix() {
        let c = version_currency_verdict("app currency", "0.3.0", "v0.3.0", "blazar upgrade");
        assert!(c.ok && !c.warn);
    ...
    fn unit__version_currency_verdict__running_ahead_is_ok() {
        let c = version_currency_verdict("app currency", "0.5.1", "v0.4.0", "blazar upgrade");
        assert!(c.ok && !c.warn);
    ...
    fn unit__version_currency_verdict__mixed_tag_shapes_fall_back_to_inequality() {
        ...
        assert_eq!(
            c.detail,
            "update available: b4938 (running: v1.7.5) — run: blazar whisper --install"
        );
        // Same unparseable tag on both sides = up to date, not a warn.
        let c = version_currency_verdict("app currency", "nightly", "nightly", "blazar upgrade");
        assert!(c.ok && !c.warn);
    ...
    fn unit__doctor_next_steps__daemon_models_ready() {
        ...
        assert_eq!(
            ...
            "start the daemon: blazar serve (or: systemctl start blazar)"
        );
        assert_eq!(
            doctor_next_steps(&down)[1],
            "pull a model: blazar pull <name> (find one: blazar search qwen3)"
        );
        // "10 pulled" must NOT match the zero-models wording
        let ten = vec![
            Check::ok("port", "blazar daemon already answering"),
            Check::ok("models", "10 pulled, all parse"),
        ];
        assert_eq!(doctor_next_steps(&ten), vec!["chat: blazar run <model>"]);
    }
    ...
    fn row_with_path(path: &std::path::Path) -> blazar_core::store::ModelRow {
        blazar_core::store::ModelRow {
            name: "m".into(),
            repo: "r".into(),
            quant: "Q4_K_M".into(),
    ...
    fn unit__doctor_store__healthy_quick_check_ok() {
        ...
        drop(Store::open(&d).unwrap()); // creates + migrates blazar.db
        let checks = doctor_store(&d);
        assert_eq!(checks.len(), 1);
        assert!(checks[0].ok && !checks[0].warn, "{}", checks[0].detail);
        assert!(
    ...
    fn unit__cuda_opportunity_rows__nudge_only_for_non_cuda_llamacpp() {
        ...
        assert!(rows[0].detail.contains("blazar engine build cuda"));
        assert!(rows[0].detail.contains("blazar engine install"));
    ...
    fn unit__cuda_opportunity_rows__missing_toolchain_names_bins() {
        ...
        assert!(rows[1].detail.contains("blazar engine install"));
    }
    ...
    fn unit__ps_rows_using_engine__matches_only_rows_of_that_engine() {
        let ps = serde_json::json!({
            "models": [
                {"name": "qwen2.5-0.5b", "blazar_engine": "b11026-cuda"},
                {"name": "amd.instella", "blazar_engine": "fork-acme_llama.cpp-7a3c74eb-cuda"},
                {"name": "no-engine-row"}
            ]
        });
