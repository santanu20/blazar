...
One Rust binary — `pallama` — that turns any machine into a complete local inference server. It orchestrates the three best open engines, unmodified and side-by-side: [llama.cpp](https://github.com/ggml-org/llama.cpp) `llama-server` (GGUF models), [mistral.rs](https://github.com/EricLBuehler/mistral.rs) (HF-style safetensors) and [SGLang](https://github.com/sgl-project/sglang) (safetensors + AWQ/GPTQ on CUDA/ROCm) — behind one gateway that speaks the **OpenAI**, **Ollama** and **Anthropic** APIs simultaneously on port **11435** (pallama's own port — never collides with a running ollama; set `port = 11434` in `config.toml` for a drop-in `OLLAMA_HOST` replacement once ollama is removed).
...
## The numbers (measured, not marketed)

Same laptop, same model (Qwen3.5-9B Q4_K_M), reproducible with one command. Full methodology, raw artifacts and caveats: [BENCHMARK.md](BENCHMARK.md).
...
pallama engine update                     # install + activate upstream llama-server (sha256-verified;
                                           ...
                                           #   (bundled cudart/cublas, no toolkit needed) and
...
OLLAMA_HOST=http://127.0.0.1:11435 ollama list   # existing ollama clients just work (point them at 11435;
                                           #   set port = 11434 in config.toml only for a drop-in after removing ollama)
```

Three API dialects on the one port — OpenAI (`/v1/chat/completions`, `/v1/completions`, `/v1/embeddings`, `/v1/rerank`, `/v1/messages`, `/v1/responses`, `/v1/batches`, `/v1/files`, `/v1/audio/transcriptions`, `/infill`, `/tokenize`…), Ollama (`/api/chat`, `/api/generate`, `/api/tags`, `/api/ps`, `/api/pull`…), and pallama-native additions (`/api/evict`, `/api/session` + session bank, `/api/keys`, `/api/why`, `/api/watch`, `/.well-known/pallama` capability discovery). Full route table with payloads and error codes: [docs/4.API_SPEC](docs/4.API_SPEC.md).
...
Every row below is a real, long-standing ollama complaint with pallama's root-cause resolution. No workarounds — different architecture.
...
## Engine routing (short version)

One active engine serves at a time (`pallama engine use`), but models come in formats engines digest differently. Routing is on by default and picks the lane per model at spawn time: quantized safetensors (AWQ/GPTQ/FP8) → sglang only; GGUF → llama.cpp (mistral.rs as alternate); plain safetensors → sglang or mistral.rs per `policy` (quality / latency / throughput). `mode = "manual"` keeps one engine for everything (byte-identical to pre-routing behavior); a per-model `[model_overrides] engine = ...` pin wins over both modes. Both API dialects route identically; `pallama list`, `/v1/models`, and `/api/tags` all show the resolved engine per model; nothing-can-serve is a teaching error, never a guess. Details + decision table: [docs/2.ARCHITECTURE](docs/2.ARCHITECTURE.md) and [docs/10.SCIENTIFIC](docs/10.SCIENTIFIC.md).
...
Load-bearing ideas: **capability manifest** (engines are probed after install; the profile compiler emits only flags the installed build supports — engine drift becomes a data problem), **prebuilt CUDA overlay** (upstream sources compiled with `-DGGML_CUDA=ON`, bundled runtimes, no fork), **bytes-based VRAM admission** (heterogeneous models co-reside by actual bytes, not a count heuristic), **eviction ladder** (child-native sleep at `idle_sleep_secs` → SIGTERM at `idle_timeout_secs`; nothing burns VRAM forever, nothing dies mid-request), **zero-tax proxy** (OpenAI traffic forwarded byte-for-byte; client disconnect aborts the upstream request and frees the slot), **single-pid signals only**. Full walkthrough: [docs/2.ARCHITECTURE](docs/2.ARCHITECTURE.md).

## Quality

764 tests (compiler tables, wiremock network suites, engine install cycles with a real stub engine, supervisor lifecycle integration, full gateway round-trips over both APIs incl. sentinel suites); `cargo clippy --workspace --all-targets -- -D warnings` clean; live E2E harnesses with a real engine (`scripts/validate.py`, `scripts/bench_matrix.py`). Details: [docs/7.SETUP](docs/7.SETUP.md).
