...
| `search <query> [--format F] [--json]` | HF model search across every weight format (downloads, likes, format, sizes): `--format gguf` (default), `safetensors`, `awq`, `gptq`, `fp8`, `mlx`, `onnx`, … — any Hub tag, or `any`/`all` for no filter; `--json` emits one JSON object per row (JSONL) for scripting |
