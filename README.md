...
| `ps [--reset]` | Live instances: state, ctx, **GPU offload + card** (`full@<card>`/`partial@<card>`/`cpu`/`auto` — silent CPU fallback is never silent, and you always see which card holds the model), in-flight, endpoint |
